...
${varyBody}
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/texture/rtt.ts
  init_samplers();
  init_render_target();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/effect/effect-renderer.ts
  init_typed_arrays();
  init_gpu_flags();
  init_render_target();
  var DEFAULT_VERTEX_WGSL = `struct EffectVertexOutput{@builtin(position) position:vec4<f32>,@location(0) uv:vec2<f32>};
...
${wrapper.options.fragmentWGSL}`
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene75.ts
  var FRAGMENT_WGSL = `@group(0) @binding(0) var<uniform> params:vec4<f32>;
@fragment fn effectFragment(input:EffectVertexOutput)->@location(0) vec4<f32>{let toto=params.x/params.x;if(toto==1.0){return vec4<f32>(0.0,1.0,0.0,1.0);}if(toto>=0.999999&&toto<1.0){return vec4<f32>(0.0,0.0,1.0,1.0);}return vec4<f32>(1.0,0.0,0.0,1.0);}`;
  var SCENE_CLEAR_COLOR = { r: 51 / 255, g: 51 / 255, b: 76 / 255, a: 1 };
  async function main() {
