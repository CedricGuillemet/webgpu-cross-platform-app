...
(() => {
  ...
  function mat4MultiplyInto(dst, d, a, i, b2, j) {
    ...
    let b0 = b2[j], b1 = b2[j + 1], b22 = b2[j + 2], b3 = b2[j + 3];
    dst[d] = a0 * b0 + a4 * b1 + a8 * b22 + a12 * b3;
    dst[d + 1] = a1 * b0 + a5 * b1 + a9 * b22 + a13 * b3;
    dst[d + 2] = a2 * b0 + a6 * b1 + a10 * b22 + a14 * b3;
    dst[d + 3] = a3 * b0 + a7 * b1 + a11 * b22 + a15 * b3;
    b0 = b2[j + 4];
    b1 = b2[j + 5];
    b22 = b2[j + 6];
    b3 = b2[j + 7];
    dst[d + 4] = a0 * b0 + a4 * b1 + a8 * b22 + a12 * b3;
    dst[d + 5] = a1 * b0 + a5 * b1 + a9 * b22 + a13 * b3;
    dst[d + 6] = a2 * b0 + a6 * b1 + a10 * b22 + a14 * b3;
    dst[d + 7] = a3 * b0 + a7 * b1 + a11 * b22 + a15 * b3;
    b0 = b2[j + 8];
    b1 = b2[j + 9];
    b22 = b2[j + 10];
    b3 = b2[j + 11];
    dst[d + 8] = a0 * b0 + a4 * b1 + a8 * b22 + a12 * b3;
    dst[d + 9] = a1 * b0 + a5 * b1 + a9 * b22 + a13 * b3;
    dst[d + 10] = a2 * b0 + a6 * b1 + a10 * b22 + a14 * b3;
    dst[d + 11] = a3 * b0 + a7 * b1 + a11 * b22 + a15 * b3;
    b0 = b2[j + 12];
    b1 = b2[j + 13];
    b22 = b2[j + 14];
    b3 = b2[j + 15];
    dst[d + 12] = a0 * b0 + a4 * b1 + a8 * b22 + a12 * b3;
    dst[d + 13] = a1 * b0 + a5 * b1 + a9 * b22 + a13 * b3;
    dst[d + 14] = a2 * b0 + a6 * b1 + a10 * b22 + a14 * b3;
    dst[d + 15] = a3 * b0 + a7 * b1 + a11 * b22 + a15 * b3;
  }
  ...
  function sortTransparentBindings(task, camera) {
    ...
    for (const b2 of arr) {
      const wc = b2.renderable._worldCenter;
      b2._sortDistance = wc ? wc[0] * v[2] + wc[1] * v[6] + wc[2] * v[10] + v[14] : 0;
    }
    arr.sort((a, b2) => b2._sortDistance - a._sortDistance || a.renderable.order - b2.renderable.order);
  }
  function buildBindings(task, eng, targetSignature) {
    ...
    for (const r2 of task._renderables) {
      const binding = r2.bind(eng, targetSignature);
      if (r2.isTransparent || r2._transmissive) {
    ...
    opaque.sort((a, b2) => a.renderable.order - b2.renderable.order);
    direct.sort((a, b2) => a.renderable.order - b2.renderable.order);
    task._opaqueBundles.length = 0;
  ...
  function drawList(enc, list, engine) {
    ...
    let draws = 0;
    for (const b2 of list) {
      const mesh = b2.renderable.mesh;
      if (mesh && mesh.visible === false) {
        continue;
      }
      if (b2.pipeline !== lp) {
        enc.setPipeline(b2.pipeline);
        lp = b2.pipeline;
      }
      draws += b2.draw(enc, engine);
    }
    return draws;
  }
      ...
      if (result.animationGroups?.length) {
        ...
        ctx._beforeRender.push((deltaMs) => {
          for (const g2 of groups) {
            if (!g2._stopped && g2._ctrl) {
              g2._ctrl.tick(deltaMs, engine);
            }
          }
        });
      }
  ...
  async function buildScene(scene) {
    ...
    while (ctx._deferredBuilders.length > 0) {
      ...
      await Promise.all(builders.map(async (b2) => b2()));
    }
  ...
  function createEulerProxy(rq) {
    ...
    const apply = (x, y, z) => {
      ...
      ey = y;
      ez = z;
      const [a, b2, c, d] = eulerToQuat(x, y, z);
      rq.set(a, b2, c, d);
      syncedVersion = rq.version;
    };
    return {
  ...
  function parseNodeMaterialSource(source) {
    ...
    for (const b2 of blocks2.values()) {
      if (b2.className !== "InputBlock") {
        continue;
      }
      const mode = b2.serialized["mode"] ?? b2.serialized["_mode"];
      if (mode === 0 || mode === void 0) {
        if (typeof b2.serialized["systemValue"] === "number") {
          continue;
        }
        if (b2.name) {
          namedInputs.set(b2.name, b2.id);
        }
      }
    }
    const rawAlpha = raw;
  ...
  function findBlockByClassName(graph, className) {
    for (const b2 of graph.blocks.values()) {
      if (b2.className === className) {
        return b2;
      }
    }
    return null;
  ...
  function widerType(a, b2) {
    const ra = RANK[a] || 0;
    const rb = RANK[b2] || 0;
    const r2 = Math.max(ra, rb);
    if (!r2) {
      throw new Error(`NodeMaterial: cannot pick common numeric type from ${a} and ${b2}`);
    }
    return RANK_TYPE[r2];
  }
  function binaryEmitter(className, op, leftName = "left", rightName = "right") {
    return {
      ...
      emit(block, _outputName, stage, state, ctx) {
        ...
        const r2 = ctx.resolve(block, rightName, stage, state);
        const t = widerType(l.type, r2.type);
        const lc = ctx.cast(l, t).expr;
        const rc = ctx.cast(r2, t).expr;
  ...
  var init_add_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/add-block.ts"() {
      "use strict";
      init_math_factory();
      emitter4 = binaryEmitter("AddBlock", (l, r2) => `${l} + ${r2}`);
    }
  });
  ...
  var init_subtract_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/subtract-block.ts"() {
      "use strict";
      init_math_factory();
      emitter5 = binaryEmitter("SubtractBlock", (l, r2) => `${l} - ${r2}`);
    }
  });
  ...
  var init_multiply_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/multiply-block.ts"() {
      "use strict";
      init_math_factory();
      emitter6 = binaryEmitter("MultiplyBlock", (l, r2) => `${l} * ${r2}`);
    }
  });
  ...
  var init_min_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/min-block.ts"() {
      "use strict";
      init_math_factory();
      emitter7 = binaryEmitter("MinBlock", (l, r2) => `min(${l}, ${r2})`);
    }
  });
  ...
  var init_max_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/max-block.ts"() {
      "use strict";
      init_math_factory();
      emitter8 = binaryEmitter("MaxBlock", (l, r2) => `max(${l}, ${r2})`);
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/dot-block.ts"() {
      ...
      emitter11 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const r2 = ctx.resolve(block, "right", stage, state);
          const t = widerType(l.type, r2.type);
          const lc = ctx.cast(l, t).expr;
          const rc = ctx.cast(r2, t).expr;
...
`;
  ...
  var init_divide_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/divide-block.ts"() {
      "use strict";
      init_math_factory();
      emitter47 = binaryEmitter("DivideBlock", (l, r2) => `${l} / ${r2}`);
    }
  });
  ...
  var init_mod_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/mod-block.ts"() {
      "use strict";
      init_math_factory();
      emitter48 = binaryEmitter("ModBlock", (l, r2) => `${l} % ${r2}`);
    }
  });
  ...
  function conditionExpr(condition, a, b2) {
    switch (condition) {
      case 0:
        return `${a} == ${b2}`;
      case 1:
      ...
      case 5:
        return `${a} >= ${b2}`;
      case 6:
        return `(((${a} + ${b2}) - 2.0 * floor((${a} + ${b2}) / 2.0)) > 0.0)`;
      case 7:
        return `(min(${a} + ${b2}, 1.0) > 0.0)`;
      case 8:
        return `(${a} * ${b2} > 0.0)`;
      default:
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/conditional-block.ts"() {
      ...
      emitter59 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const b2 = ctx.cast(ctx.resolve(block, "b", stage, state), "f32").expr;
          ...
          return { expr: `select(${f}, ${t}, ${conditionExpr(condition, a, b2)})`, type: outputType };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/color-merger.ts"() {
      ...
      emitter63 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const r2 = tryResolve2(block, "r", stage, state, ctx);
          const g2 = tryResolve2(block, "g", stage, state, ctx);
          const b2 = tryResolve2(block, "b", stage, state, ctx);
          const rExpr = r2 ? ctx.cast(r2, "f32").expr : "0.0";
          const gExpr = g2 ? ctx.cast(g2, "f32").expr : "0.0";
          const bExpr = b2 ? ctx.cast(b2, "f32").expr : "0.0";
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/gradient-block.ts"() {
      ...
      emitter66 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const gradient = ctx.resolve(block, "gradient", stage, state);
          const g2 = gradient.type === "f32" ? gradient.expr : `((${gradient.expr}).x)`;
          ...
          for (let i = 1; i < steps.length; i++) {
            ...
            state[stage].body.push(`${tempPosition} = clamp((${g2} - ${formatFloat2(previous.step)}) / (${formatFloat2(current.step)} - ${formatFloat2(previous.step)}), 0.0, 1.0);`);
  ...
  function buildNodeMeshRenderables(scene, meshes, materialOverride) {
    ...
    for (const [material, matMeshes] of byMaterial) {
      ...
      const drawPacket = (pass, pkt) => {
        const g2 = pkt._mesh._gpu;
        for (let i = 0; i < attrNames.length; i++) {
          const buf = getAttrBuffer(engine, g2, attrNames[i]);
          pass.setVertexBuffer(i, buf);
        }
        pass.setIndexBuffer(g2.indexBuffer, g2.indexFormat);
        pass.setBindGroup(1, pkt._meshBG);
        pass.drawIndexed(g2.indexCount);
      };
...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene88-nme.ts
  var TARGET_VERTEX = 1;
  ...
  var initialColor = inputBlock("initialLoopColor", TYPE_VECTOR3, [0.015, 0.02, 0.035]);
  var loop = addBlock(
    "LoopBlock",
    "AccumulateBandsLoop",
    TARGET_NEUTRAL,
  ...
  var read = addBlock("StorageReadBlock", "ReadAccumulatedColor", TARGET_NEUTRAL, [input("loopID", { id: loop, output: "loopID" })], [output("value")]);
  var five = inputBlock("loopCount", TYPE_FLOAT, 5);
  var one2 = inputBlock("one", TYPE_FLOAT, 1);
  var rBase = inputBlock("rBase", TYPE_FLOAT, 0.035);
  var rSlope = inputBlock("rSlope", TYPE_FLOAT, 0.075);
  var gBase = inputBlock("gBase", TYPE_FLOAT, 0.025);
  var gSlope = inputBlock("gSlope", TYPE_FLOAT, 0.055);
  var bBase = inputBlock("bBase", TYPE_FLOAT, 0.035);
  var bSlope = inputBlock("bSlope", TYPE_FLOAT, 0.03);
  var indexNorm = divide("IndexNorm", { id: loop, output: "index" }, { id: five, output: "output" });
  var mask = addBlock(
    "StepBlock",
    "UvBandMask",
    TARGET_NEUTRAL,
  ...
  var invIndexNorm = subtract("OneMinusIndexNorm", { id: one2, output: "output" }, { id: indexNorm, output: "output" });
  var r = add("BandR", { id: rBase, output: "output" }, { id: multiply("BandRSlope", { id: indexNorm, output: "output" }, { id: rSlope, output: "output" }), output: "output" });
  var g = add("BandG", { id: gBase, output: "output" }, { id: multiply("BandGSlope", { id: invIndexNorm, output: "output" }, { id: gSlope, output: "output" }), output: "output" });
  var b = add("BandB", { id: bBase, output: "output" }, { id: multiply("BandBSlope", { id: indexNorm, output: "output" }, { id: bSlope, output: "output" }), output: "output" });
  var bandColor = addBlock(
    "VectorMergerBlock",
    "BandColor",
    TARGET_NEUTRAL,
    [
  ...
  var contribution = multiply("MaskedBandColor", { id: bandColor, output: "xyz" }, { id: mask, output: "output" });
  var nextAccum = add("NextAccumulatedColor", { id: read, output: "value" }, { id: contribution, output: "output" });
  ...
  var fragmentOutput = addBlock("FragmentOutputBlock", "FragmentOutput", TARGET_FRAGMENT, [input("rgba"), input("rgb", { id: loop, output: "output" }), input("a")], [], {
    convertToGammaSpace: false,
    convertToLinearSpace: false,
    useLogarithmicDepth: false
  });
  var SCENE88_NME_JSON = {
    tags: null,
    ignoreAlpha: false,
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene88.ts
  async function main() {
