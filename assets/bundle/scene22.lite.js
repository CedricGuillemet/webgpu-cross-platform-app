...
@fragment fn fs(v:V)->@location(0)vec4f{return textureSample(t,s,v.u);}`;
  ...
  function createRenderTaskTransmission(task, engine) {
    ...
    const generateMipmaps2 = mipLevelCount > 1;
...
return vec4<f32>(depthSM, 1.0, 1.0, 1.0);`;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/create-torus.ts
  init_typed_arrays();
  function createTorusData(opts = {}) {
  ...
  function applyHeightmap(ground, heightmapData, hmWidth, hmHeight, subdivisions, minHeight, maxHeight) {
    const cols = subdivisions + 1;
    const rows = cols;
  ...
  async function createGroundFromHeightMap(heightmapUrl, opts = {}) {
    const subdivisions = opts.subdivisions ?? 1;
  ...
  async function createGroundFromHeightMap2(engine, url, options) {
    const data = await createGroundFromHeightMap(url, options);
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene22.ts
  async function main() {
    ...
    const ground = await createGroundFromHeightMap2(engine, "https://playground.babylonjs.com/textures/heightMap.png", {
      width: 100,
      height: 100,
      subdivisions: 100,
      minHeight: 0,
