...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene89-nme.ts
  var TARGET_VERTEX = 1;
  ...
  var initialStorage = inputBlock("initialStorage", TYPE_VECTOR4, [0.035, 0.045, 0.055, 1]);
  var loop = addBlock(
    "LoopBlock",
    "StorageFeedbackLoop",
    TARGET_NEUTRAL,
  ...
  var read = addBlock("StorageReadBlock", "ReadLoopStorage", TARGET_NEUTRAL, [input("loopID", { id: loop, output: "loopID" })], [output("value")]);
  var readSplit = addBlock(
    "VectorSplitterBlock",
    "SplitLoopStorage",
    TARGET_NEUTRAL,
  ...
  var six = inputBlock("loopCount", TYPE_FLOAT, 6);
  var one2 = inputBlock("one", TYPE_FLOAT, 1);
  var rBase = inputBlock("rBase", TYPE_FLOAT, 0.025);
  var rSlope = inputBlock("rSlope", TYPE_FLOAT, 0.055);
  var gBase = inputBlock("gBase", TYPE_FLOAT, 0.02);
  var gSlope = inputBlock("gSlope", TYPE_FLOAT, 0.045);
  var bBase = inputBlock("bBase", TYPE_FLOAT, 0.012);
  var bSlope = inputBlock("bSlope", TYPE_FLOAT, 0.02);
  var bFeedbackScale = inputBlock("bFeedbackScale", TYPE_FLOAT, 0.08);
  var indexPlusOne = add("IndexPlusOne", { id: loop, output: "index" }, { id: one2, output: "output" });
  var indexNorm = divide("IndexNorm", { id: indexPlusOne, output: "output" }, { id: six, output: "output" });
  var mask = addBlock(
    "StepBlock",
    "StorageBandMask",
    TARGET_NEUTRAL,
  ...
  var rWeight = add("RWeight", { id: rBase, output: "output" }, { id: multiply("RWeightSlope", { id: indexNorm, output: "output" }, { id: rSlope, output: "output" }), output: "output" });
  var gWeight = add("GWeight", { id: gBase, output: "output" }, { id: multiply("GWeightSlope", { id: invIndexNorm, output: "output" }, { id: gSlope, output: "output" }), output: "output" });
  var bSeed = add("BSeed", { id: bBase, output: "output" }, { id: multiply("BSeedSlope", { id: indexNorm, output: "output" }, { id: bSlope, output: "output" }), output: "output" });
  var bFeedback = multiply("BFeedback", { id: readSplit, output: "x" }, { id: bFeedbackScale, output: "output" });
  var bWeight = add("BWeight", { id: bSeed, output: "output" }, { id: bFeedback, output: "output" });
  var rContribution = multiply("RContribution", { id: mask, output: "output" }, { id: rWeight, output: "output" });
  var maskedGWeight = multiply("MaskedGWeight", { id: mask, output: "output" }, { id: gWeight, output: "output" });
  var gContribution = multiply("GContribution", { id: maskedGWeight, output: "output" }, { id: uvSplit, output: "y" });
  var bContribution = multiply("BContribution", { id: mask, output: "output" }, { id: bWeight, output: "output" });
  var nextR = add("NextR", { id: readSplit, output: "x" }, { id: rContribution, output: "output" });
  var nextG = add("NextG", { id: readSplit, output: "y" }, { id: gContribution, output: "output" });
  var nextB = add("NextB", { id: readSplit, output: "z" }, { id: bContribution, output: "output" });
  var nextStorage = addBlock(
    "VectorMergerBlock",
    "NextLoopStorage",
    TARGET_NEUTRAL,
    [
  ...
  var fragmentOutput = addBlock("FragmentOutputBlock", "FragmentOutput", TARGET_FRAGMENT, [input("rgba", { id: loop, output: "output" }), input("rgb"), input("a")], [], {
    convertToGammaSpace: false,
    convertToLinearSpace: false,
    useLogarithmicDepth: false
  });
  var SCENE89_NME_JSON = {
    tags: null,
    ignoreAlpha: false,
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene89.ts
  async function main() {
