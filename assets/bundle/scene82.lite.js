...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene82-nme.ts
  var TARGET_VERTEX = 1;
  ...
  function selectColor(name, a, aOutput, b, bOutput, condition, trueBlock, trueOutput, falseBlock, falseOutput) {
    return addBlock(
      "ConditionalBlock",
      name,
      TARGET_NEUTRAL,
  ...
  var transform = addBlock("TransformBlock", "Transform", TARGET_VERTEX, [input("vector", { id: position, output: "output" }), input("transform", { id: wvp, output: "output" })], [output("output"), output("xyz")], {
    complementZ: 0,
    complementW: 1
  });
  ...
  var half = inputBlock("quadrantHalf", TYPE_FLOAT, 0.5);
  var one2 = inputBlock("one", TYPE_FLOAT, 1);
  var blueBias = inputBlock("blueBias", TYPE_FLOAT, 0.88);
  var yellowBias = inputBlock("yellowBias", TYPE_FLOAT, 0.18);
  var simplexZ = inputBlock("simplexSeedZ", TYPE_FLOAT, 0.37);
  var worleyZ = inputBlock("worleySeedZ", TYPE_FLOAT, 0.61);
  var cloudScaleK = inputBlock("cloudScale", TYPE_FLOAT, 5.25);
  var simplexScaleK = inputBlock("simplexScale", TYPE_FLOAT, 7.4);
  var voronoiDensity = inputBlock("voronoiDensity", TYPE_FLOAT, 8);
  var voronoiOffset = inputBlock("voronoiOffset", TYPE_FLOAT, 1.125);
  var worleyScaleK = inputBlock("worleyScale", TYPE_FLOAT, 4.6);
  var worleyJitter = inputBlock("worleyJitter", TYPE_FLOAT, 0.72);
  var cloudSeed = scale("CloudSeedScale", uv, "output", cloudScaleK);
  var cloudChaos = inputBlock("cloudChaos", TYPE_VECTOR2, [0.13, 0.31]);
  var cloudOffsetX = inputBlock("cloudOffsetX", TYPE_FLOAT, 2);
  var cloudOffsetY = inputBlock("cloudOffsetY", TYPE_FLOAT, -1.25);
  var cloudNoise = addBlock(
    "CloudBlock",
    "CloudNoise",
    TARGET_NEUTRAL,
  ...
  var cloudColor = color("CloudBlue", cloudNoise, "output", cloudNoise, "output", blueBias, "output");
  var simplexSeedUv = scale("SimplexSeedUv", uv, "output", simplexScaleK);
  var simplexSeed = vector3("SimplexSeed3", simplexSeedUv, "output", simplexZ);
  var simplexRaw = addBlock("SimplexPerlin3DBlock", "SimplexNoise", TARGET_NEUTRAL, [input("seed", { id: simplexSeed, output: "xyz" })], [output("output")]);
  var simplexPlusOne = add("SimplexPlusOne", simplexRaw, "output", one2, "output");
  var halfScale = inputBlock("halfScale", TYPE_FLOAT, 0.5);
  var simplex01 = scale("SimplexTo01", simplexPlusOne, "output", halfScale);
  var simplexColor = color("SimplexRed", simplex01, "output", yellowBias, "output", simplex01, "output");
  var voronoi = addBlock("VoronoiNoiseBlock", "VoronoiNoise", TARGET_NEUTRAL, [input("seed", { id: uv, output: "output" }), input("offset", { id: voronoiOffset, output: "output" }), input("density", { id: voronoiDensity, output: "output" })], [output("output"), output("cells")]);
  var voronoiColor = color("VoronoiGreen", voronoi, "output", voronoi, "cells", yellowBias, "output");
  var worleySeedUv = scale("WorleySeedUv", uv, "output", worleyScaleK);
  var worleySeed = vector3("WorleySeed3", worleySeedUv, "output", worleyZ);
  var worley = addBlock("WorleyNoise3DBlock", "WorleyNoise", TARGET_NEUTRAL, [input("seed", { id: worleySeed, output: "xyz" }), input("jitter", { id: worleyJitter, output: "output" })], [output("output"), output("x"), output("y")], {
    manhattanDistance: false
  });
  var worleyColor = color("WorleyPurple", worley, "x", yellowBias, "output", worley, "y");
  var bottomRow = selectColor("BottomCloudOrSimplex", uvSplit, "x", half, "output", 2, cloudColor, "rgb", simplexColor, "rgb");
  var topRow = selectColor("TopVoronoiOrWorley", uvSplit, "x", half, "output", 2, voronoiColor, "rgb", worleyColor, "rgb");
  var finalColor = selectColor("FinalTopOrBottom", uvSplit, "y", half, "output", 3, topRow, "output", bottomRow, "output");
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene82.ts
  async function main() {
