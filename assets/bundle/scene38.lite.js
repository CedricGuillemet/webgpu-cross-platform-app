...
${varyBody}
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/create-disc.ts
  init_typed_arrays();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/create-extrude.ts
  function rodrigues2(v, k, angle) {
    const c = Math.cos(angle);
    const s = Math.sin(angle);
    const dot = k.x * v.x + k.y * v.y + k.z * v.z;
    const crossX = k.y * v.z - k.z * v.y;
  ...
  function createExtrudeShapeData(options) {
    ...
    for (let i = 0; i < curve.length; i++) {
      ...
      for (let p = 0; p < shape.length; p++) {
        ...
        const rotated = rodrigues2(planed, t, angle);
        shapePath.push({
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene38.ts
  async function main() {
