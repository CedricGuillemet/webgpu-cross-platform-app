    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/rotate2d-block.ts"() {
      ...
      emitter70 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const angle2 = ctx.cast(ctx.resolve(block, "angle", stage, state), "f32");
          return {
            expr: `vec2<f32>(cos(${angle2.expr}) * (${input2.expr}).x - sin(${angle2.expr}) * (${input2.expr}).y, sin(${angle2.expr}) * (${input2.expr}).x + cos(${angle2.expr}) * (${input2.expr}).y)`,
            type: "vec2f"
          };
        }
      };
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/twirl-block.ts"() {
      ...
      emitter86 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const angle2 = ctx.temp(state, "twirlAngle");
          const result = ctx.temp(state, "twirl");
          state.fragment.body.push(`let ${delta} = ${input2.expr} - ${center.expr};`);
          state.fragment.body.push(`let ${angle2} = ${strength.expr} * length(${delta});`);
          state.fragment.body.push(
            `let ${result} = vec2<f32>(cos(${angle2}) * ${delta}.x - sin(${angle2}) * ${delta}.y, sin(${angle2}) * ${delta}.x + cos(${angle2}) * ${delta}.y) + ${center.expr} + ${offset.expr};`
          );
...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene81-nme.ts
  var TARGET_VERTEX = 1;
  ...
  var panSpeed = inputBlock("panSpeed", TYPE_VECTOR2, [0.37, -0.22]);
  var frozenTime = inputBlock("frozenTime", TYPE_FLOAT, 1.75);
  var panner = addBlock("PannerBlock", "FrozenPanner", TARGET_NEUTRAL, [input("uv", { id: uv, output: "output" }), input("speed", { id: panSpeed, output: "output" }), input("time", { id: frozenTime, output: "output" })], [output("output")]);
  var angle = inputBlock("rotateRadians", TYPE_FLOAT, 0.72);
  var rotate = addBlock("Rotate2dBlock", "RotatePannedUV", TARGET_NEUTRAL, [input("input", { id: panner, output: "output" }), input("angle", { id: angle, output: "output" })], [output("output")]);
  var uvAtlas = textureBlock("TextureBlock", "AtlasUV", [input("uv", { id: rotate, output: "output" }), input("source"), input("layer"), input("lod")]);
  var triSharpness = inputBlock("triSharpness", TYPE_FLOAT, 4);
  var tri = textureBlock(
    "TriPlanarBlock",
    "TriAtlas",
  ...
  var biSharpness = inputBlock("biSharpness", TYPE_FLOAT, 6);
  var bi = textureBlock("BiPlanarBlock", "BiAtlas", [input("position", { id: position, output: "output" }), input("normal", { id: normal, output: "output" }), input("sharpness", { id: biSharpness, output: "output" }), input("source"), input("sourceY")]);
  var uvPlusTri = addBlock("AddBlock", "UvPlusTri", TARGET_NEUTRAL, [input("left", { id: uvAtlas, output: "rgb" }), input("right", { id: tri, output: "rgb" })], [output("output")]);
  var allThree = addBlock("AddBlock", "AddBi", TARGET_NEUTRAL, [input("left", { id: uvPlusTri, output: "output" }), input("right", { id: bi, output: "rgb" })], [output("output")]);
  var oneThird = inputBlock("oneThird", TYPE_FLOAT, 1 / 3);
  var finalColor = addBlock("ScaleBlock", "AverageProjectionColors", TARGET_NEUTRAL, [input("input", { id: allThree, output: "output" }), input("factor", { id: oneThird, output: "output" })], [output("output")]);
