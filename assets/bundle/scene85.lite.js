...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene85-nme.ts
  var TARGET_VERTEX = 1;
  ...
  function transform(name, vector, vectorOutput, matrix2) {
    return addBlock("TransformBlock", name, TARGET_VERTEX, [input("vector", { id: vector, output: vectorOutput }), input("transform", { id: matrix2, output: "output" })], [output("output"), output("xyz")], {
      complementZ: 0,
      complementW: 1
    });
  }
  function vector4(name, x, xOut, y, yOut, z, w) {
  ...
  function scale(name, source, factorValue) {
  ...
  function dot(name, left, leftOutput, right, rightOutput) {
  ...
  var matrixCol0 = inputBlock("matrixCol0", TYPE_VECTOR4, [1.2, 0.2, 0, 0]);
  var matrixCol1 = inputBlock("matrixCol1", TYPE_VECTOR4, [-0.15, 0.9, 0.25, 0]);
  var matrixCol2 = inputBlock("matrixCol2", TYPE_VECTOR4, [0.35, -0.1, 1.1, 0]);
  var matrixCol3 = inputBlock("matrixCol3", TYPE_VECTOR4, [0.05, 0.1, 0.2, 1]);
  var matrix = addBlock(
    "MatrixBuilder",
    "MatrixBuilder",
    TARGET_NEUTRAL,
  ...
  var localTransform = transform("MatrixVertexTransform", position, "output", matrix);
  var clipTransform = transform("TransformWVP", localTransform, "output", wvp);
  var vertexOutput = addBlock("VertexOutputBlock", "VertexOutput", TARGET_VERTEX, [input("vector", { id: clipTransform, output: "output" })], []);
  var probeX = inputBlock("matrixProbeX", TYPE_FLOAT, 0.35);
  var probeY = inputBlock("matrixProbeY", TYPE_FLOAT, 0.65);
  var probeVec4 = vector4("MatrixProbeVec4", probeX, "output", probeY, "output", 0.35, 1);
  var transpose = addBlock("MatrixTransposeBlock", "MatrixTranspose", TARGET_NEUTRAL, [input("input", { id: matrix, output: "output" })], [output("output")]);
  var matrixSplit = addBlock("MatrixSplitterBlock", "MatrixSplitter", TARGET_NEUTRAL, [input("input", { id: transpose, output: "output" })], [output("row0"), output("row1"), output("row2"), output("row3"), output("col0"), output("col1"), output("col2"), output("col3")]);
  var rowDot = clamp("RedFromTransposedRow", scale("ScaleRedDot", dot("DotRow0Probe", matrixSplit, "row0", probeVec4, "xyzw"), 0.65));
  var colDot = clamp("GreenFromSplitColumn", scale("ScaleGreenDot", dot("DotCol1Probe", matrixSplit, "col1", probeVec4, "xyzw"), 0.7));
  var determinant = addBlock("MatrixDeterminantBlock", "MatrixDeterminant", TARGET_NEUTRAL, [input("input", { id: matrix, output: "output" })], [output("output")]);
  var determinantBlue = clamp("BlueFromDeterminant", scale("ScaleDeterminant", determinant, 0.55));
  var rgb = color("FragmentColor", rowDot, colDot, determinantBlue);
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene85.ts
  async function main() {
