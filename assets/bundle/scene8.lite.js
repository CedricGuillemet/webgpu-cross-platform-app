...
@fragment fn fs(v:V)->@location(0)vec4f{return textureSample(t,s,v.u);}`;
  ...
  function createRenderTaskTransmission(task, engine) {
    ...
    const generateMipmaps = mipLevelCount2 > 1;
...
`;
  ...
  var scene_size_exports = {};
  __export(scene_size_exports, {
  ...
  var SKYBOX_POS_BUFFER, _cmPipelines, _cmLayouts, _cmCachedDevice;
  ...
  var SKY_MESH_UNIFORM_SIZE, SKYBOX_POS_BUFFER2, _skyPipelines, _skyLayout, _skyCachedDevice;
...
`;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/math/spherical-harmonics.ts
  init_typed_arrays();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/loader-hdr/load-hdr.ts
  init_gpu_pool();

  // ../../../Babylon-Lite/packages/babylon-lite/src/loader-hdr/hdr-parser.ts
  init_typed_arrays();
  ...
  function equirectToCubemapGPU(engine, hdr, faceSize) {
    ...
    const module = device.createShaderModule({ code: hdr_equirect_to_cube_compute_default });
  ...
  function prefilterCubemapGPU(engine, srcCube, faceSize, mipCount) {
    ...
    const pipeline = device.createComputePipeline({
      layout: "auto",
      compute: { module: device.createShaderModule({ code: hdr_prefilter_cube_compute_default }), entryPoint: "main" }
    });
  ...
  var _brdfPipeline = null;
  var _brdfPipelineDevice = null;
  function generateBrdfLut(engine) {
    const device = engine._device;
    if (!_brdfPipeline || _brdfPipelineDevice !== device) {
      _brdfPipeline = device.createComputePipeline({
        layout: "auto",
        compute: { module: device.createShaderModule({ code: hdr_brdf_lut_compute_default }), entryPoint: "main" }
      });
  ...
  async function loadHdrEnvironment(scene, url, options) {
    ...
    scene._deferredBuilders.push(async () => {
      if (useHdr && textures.specularCubeView) {
        const { computeSceneSize: computeSceneSize2 } = await Promise.resolve().then(() => (init_scene_size(), scene_size_exports));
        const { skyboxSize: autoSkyboxSize, rootPosition } = computeSceneSize2(scene, options?.skyboxSize);
        ...
        scene._renderables.push(buildHdrSkyboxRenderable2(scene, textures, autoSkyboxSize / 2, rootPosition, primaryColor));
      }
      if (!useHdr || !skipGround) {
        const primaryColor = scene.environmentPrimaryColor ?? [0.08697355964132344, 0.08697355964132344, 0.2122208331110881];
        const { computeSceneSize: computeSceneSize2 } = await Promise.resolve().then(() => (init_scene_size(), scene_size_exports));
        const { groundSize, skyboxSize: autoSkyboxSize, rootPosition } = computeSceneSize2(scene, options?.skyboxSize);
        if (!useHdr) {
          ...
          scene._renderables.push(buildSolidSkyboxRenderable2(scene, textures, autoSkyboxSize / 2, rootPosition, primaryColor));
        }
        if (!skipGround) {
          ...
          scene._renderables.push(await buildGroundRenderable2(engine, groundSize, rootPosition, primaryColor));
        }
      }
    });
    return textures;
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene8.ts
  async function main() {
