...
@group(1) @binding(${binding}) var<uniform> nmeGeom: NmeGeomParams;`,
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/texture/cube-texture.ts
  var cube_texture_exports = {};
  __export(cube_texture_exports, {
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/loader-babylon/bake-local-matrix.ts
  var bake_local_matrix_exports = {};
  __export(bake_local_matrix_exports, {
    bakeLocalMatrix: () => bakeLocalMatrix
  });
  function bakeLocalMatrix(positions, normals, lm) {
    const isIdentity = lm[0] === 1 && lm[1] === 0 && lm[2] === 0 && lm[4] === 0 && lm[5] === 1 && lm[6] === 0 && lm[8] === 0 && lm[9] === 0 && lm[10] === 1 && lm[12] === 0 && lm[13] === 0 && lm[14] === 0;
    if (isIdentity) {
      return;
    }
    for (let i = 0; i < positions.length; i += 3) {
  ...
  var init_bake_local_matrix = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/loader-babylon/bake-local-matrix.ts"() {
      "use strict";
    }
  });

  // ../../../Babylon-Lite/packages/babylon-lite/src/loader-babylon/parse-camera.ts
  var parse_camera_exports = {};
  __export(parse_camera_exports, {
    parseBabylonCamera: () => parseBabylonCamera
  });
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/frame-graph/copy-to-texture-task.ts
  init_gpu_flags();
  init_render_target();
  init_samplers();
  var VERTEX_WGSL = `struct V{@builtin(position)p:vec4f,@location(0)u:vec2f};
...
@fragment fn fs(v:V)->@location(0)vec4f{return textureSampleLevel(t,s,v.u,${lod.toFixed(1)});}`;
  ...
  var FRAGMENT_MSAA_WGSL = `@group(0)@binding(0)var t:texture_multisampled_2d<f32>;
...
return mix(mix(c00,c10,fr.x),mix(c01,c11,fr.x),fr.y);}`;
  var _cacheDevice = null;
  var _bglFiltering = null;
  var _bglMsaa = null;
  var _pipelines = null;
  var _shaderModules = null;
  ...
  function getOrCreateCopyPipeline(engine, targetFormat, targetSamples, multisampledSource, lodLevel) {
    ...
    let shaderModule2 = _shaderModules.get(moduleKey);
    if (!shaderModule2) {
      const fragment = multisampledSource ? FRAGMENT_MSAA_WGSL : fragmentForSingle(lodLevel);
      shaderModule2 = device.createShaderModule({ code: `${VERTEX_WGSL}
${fragment}`, label: `copy-to-texture-${moduleKey}` });
      _shaderModules.set(moduleKey, shaderModule2);
    }
    let bgl;
    if (multisampledSource) {
      _bglMsaa ?? (_bglMsaa = device.createBindGroupLayout({
        label: "copy-to-texture-msaa-bgl",
        entries: [{ binding: 0, visibility: SS.FRAGMENT, texture: { sampleType: "unfilterable-float", multisampled: true } }]
      }));
      bgl = _bglMsaa;
    } else {
      _bglFiltering ?? (_bglFiltering = device.createBindGroupLayout({
    ...
    pipeline = device.createRenderPipeline({
      ...
      vertex: { module: shaderModule2, entryPoint: "vs" },
      fragment: { module: shaderModule2, entryPoint: "fs", targets: [{ format: targetFormat }] },
      primitive: { topology: "triangle-list" },
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/camera/free-camera-controls.ts
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/loader-babylon/load-babylon.ts
  init_typed_arrays();
  init_create_standard_material();
  init_mesh();
  init_texture_2d();
  ...
  async function loadBabylon(engine, url, opts = {}) {
    ...
    if (data.materials) {
      for (const md of data.materials) {
        ...
        if (md.reflectionTexture && opts.loadTextures !== false && md.reflectionTexture.isCube) {
          ...
          texturePromises.push(
            Promise.resolve().then(() => (init_cube_texture(), cube_texture_exports)).then(
              ({ loadCubeTexture: loadCubeTexture2 }) => loadCubeTexture2(engine, baseUrl + cubeName).then((cube) => {
    ...
    if (data.lights?.length) {
      ...
      for (const ld of data.lights) {
        if (ld.type === 0 && ld.position) {
          const pl = createPointLight2([ld.position[0], ld.position[1], ld.position[2]], ld.intensity ?? 1);
          if (ld.diffuse) {
    ...
    if (data.meshes) {
      ...
      const bakeLocalMatrix2 = hasAnyLocalMatrix ? (await Promise.resolve().then(() => (init_bake_local_matrix(), bake_local_matrix_exports))).bakeLocalMatrix : null;
      for (const md of data.meshes) {
        ...
        if (md.positions && md.normals && md.indices && md.indices.length > 0) {
          ...
          if (md.localMatrix && bakeLocalMatrix2) {
            bakeLocalMatrix2(positions, normals, md.localMatrix);
          }
          let matIds = null;
          if (md.materialId) {
    ...
    const camera = camData ? (await Promise.resolve().then(() => (init_parse_camera(), parse_camera_exports))).parseBabylonCamera(camData) : void 0;
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene145.ts
  var HILLVALLEY_URL = "https://www.babylonjs.com/Scenes/hillvalley/HillValley.babylon";
  async function main() {
    ...
    const geomTaskA = createGeometryRendererTask(
      {
        ...
        textureDescriptions: [
          { type: 0 /* IRRADIANCE */ },
          { type: 1 /* WORLD_POSITION */ },
          { type: 5 /* NORMALIZED_VIEW_DEPTH */ },
          { type: 7 /* VIEW_NORMAL */ },
          { type: 8 /* WORLD_NORMAL */ },
          { type: 3 /* REFLECTIVITY */ },
          { type: 9 /* ALBEDO */ }
    ...
    const geomTaskB = createGeometryRendererTask(
      {
        ...
        textureDescriptions: [
          ...
          { type: 4 /* VIEW_DEPTH */, format: "r16float" },
          { type: 6 /* SCREENSPACE_DEPTH */ },
          { type: 10 /* LINEAR_VELOCITY */ }
        ]
      },
      engine,
      scene
