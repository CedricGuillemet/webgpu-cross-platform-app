...
`;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/material/plugin/enable-material-plugins.ts
  init_pbr_flags();
  init_standard_flags();

  // ../../../Babylon-Lite/packages/babylon-lite/src/material/plugin/plugin-bridge-shared.ts
  init_ubo_layout();
  var STAGE_FRAGMENT19 = 2;
  var STD_PLUGIN_UBO = "pluginUbo";
  var FRAG_POINT_TO_SLOTS = {
    ...
    for (const p of enabled) {
      ...
      if (samplers) {
        for (const s of samplers) {
          bindings.push(
            { _name: s.texture, _type: { _kind: "texture", _textureType: s.textureType ?? "texture_2d<f32>" }, _visibility: STAGE_FRAGMENT19 },
            { _name: s.sampler, _type: { _kind: "sampler", _samplerType: s.samplerType ?? "sampler" }, _visibility: STAGE_FRAGMENT19 }
          );
        }
      }
    }
...
` + helpers;
      bindings.unshift({ _name: STD_PLUGIN_UBO, _type: { _kind: "uniform-buffer" }, _group: "mesh", _visibility: STAGE_FRAGMENT19 });
      stdUboFields = void 0;
    }
    return {
      _fragment: {
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/material/plugin/pbr-plugin-bridge.ts
  var PLUGIN_INDEX_SHIFT = 24;
  var _sigToIndex = null;
  var _indexToEntry = null;
  var _counter = 0;
  ...
  function _indexFor(plugins) {
    ...
    if (idx === void 0) {
      idx = ++_counter;
      map.set(sig, idx);
      (_indexToEntry ?? (_indexToEntry = /* @__PURE__ */ new Map())).set(idx, { _plugins: plugins, _fragment: buildPluginFragment(plugins, idx, false)._fragment });
    }
    return idx;
  }
  var pbrPluginExt = {
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/material/plugin/std-plugin-bridge.ts
  init_standard_material();
  init_gpu_buffers();
  var PLUGIN_INDEX_SHIFT2 = 24;
  var PLUGIN_INDEX_MASK = 127;
  var _sigToIndex2 = null;
  var _indexToEntry2 = null;
  var _counter2 = 0;
  function _resetState2() {
    _sigToIndex2 = /* @__PURE__ */ new Map();
    _indexToEntry2 = /* @__PURE__ */ new Map();
    _counter2 = 0;
  }
  function _indexFor2(plugins, engine) {
    const sig = pluginSignature(plugins);
    const map = _sigToIndex2 ?? (_sigToIndex2 = /* @__PURE__ */ new Map());
    let idx = map.get(sig);
    if (idx === void 0) {
      idx = ++_counter2;
      map.set(sig, idx);
      ...
      (_indexToEntry2 ?? (_indexToEntry2 = /* @__PURE__ */ new Map())).set(idx, { _plugins: plugins, _fragment: built._fragment, _uboBuffer: uboBuffer });
    }
    return idx;
  }
  function _entryFor(plugins) {
    const idx = _sigToIndex2?.get(pluginSignature(plugins));
    return idx ? _indexToEntry2?.get(idx) : void 0;
  }
  var stdPluginExt = {
    _id: "plugin",
    _phase: "mesh",
    _feature: PLUGIN_INDEX_MASK << PLUGIN_INDEX_SHIFT2,
    _frag(features) {
      const idx = features >>> PLUGIN_INDEX_SHIFT2 & PLUGIN_INDEX_MASK;
      return _indexToEntry2?.get(idx)?._fragment ?? { _id: "plugin-0" };
    },
    _bind(mat, entries, b) {
      const plugins = mat.plugins;
      if (!plugins?.length) {
  ...
  function registerStdPlugins(meshes, engine, register) {
    ...
    for (const m of meshes) {
      ...
      if (mat?.plugins?.length && mat._buildGroup === standardGroupBuilder) {
        const idx = _indexFor2(mat.plugins, engine);
        mat._renderFeatures = { features: _computeStandardMaterialFeatures(mat) | idx << PLUGIN_INDEX_SHIFT2 };
      }
    }
  }
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene217.ts
  var blackAndWhite = {
    name: "BlackAndWhite",
