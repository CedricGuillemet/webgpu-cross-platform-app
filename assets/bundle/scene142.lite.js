...
${task._shader.fragmentWrapperWGSL ?? FRAGMENT_WRAPPER_WGSL}`;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/post-process/black-and-white.ts
  var BLACK_AND_WHITE_UNIFORM_WGSL = `struct BlackAndWhiteParams{degree:f32,p0:f32,p1:f32,p2:f32}
@group(0) @binding(2) var<uniform> blackAndWhiteParams:BlackAndWhiteParams;`;
  var BLACK_AND_WHITE_FRAGMENT_WGSL = `fn applyPostProcess(color:vec4f, uv:vec2f)->vec4f{let luminance=dot(color.rgb,vec3f(0.3,0.59,0.11));let gray=vec3f(luminance);return vec4f(mix(color.rgb,gray,clamp(blackAndWhiteParams.degree,0,1)),color.a);}`;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/post-process/anaglyph.ts
  var ANAGLYPH_EXTRA_TEXTURE_WGSL = `@group(0) @binding(2) var leftTextureSampler:texture_2d<f32>;`;
  var ANAGLYPH_FRAGMENT_WGSL = `fn applyPostProcess(color:vec4f, uv:vec2f)->vec4f{let l=textureSampleLevel(leftTextureSampler,sourceSampler,clamp(uv,vec2f(0),vec2f(1)),0);let left=vec4f(1,l.g,l.b,1);let right=vec4f(color.r,1,1,1);return vec4f(right.rgb*left.rgb,1);}`;
  function createAnaglyphPostProcessTask(config, engine, scene) {
    const task = createPostProcessTask(
      {
...
@group(0) @binding(2) var<uniform> chromaticAberrationParams:ChromaticAberrationParams;`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene142.ts
