...
${varyBody}
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/csg.ts
  init_typed_arrays();
  init_mat4_invert();
  ...
  var CsgPlane = class _CsgPlane {
    ...
    clone() {
      return new _CsgPlane(this.nx, this.ny, this.nz, this.w);
    }
    flip() {
      this.nx = -this.nx;
      this.ny = -this.ny;
  ...
  var CsgPolygon = class _CsgPolygon {
    constructor(vertices, materialSlot = 0) {
    ...
    clone() {
      return new _CsgPolygon(this.vertices.map(cloneVertex), this.materialSlot);
    }
    flip() {
  ...
  var CsgNode = class _CsgNode {
    ...
    build(polygons) {
      ...
      if (front.length > 0) {
        if (!this.front) {
          this.front = new _CsgNode();
        }
        this.front.build(front);
      }
      if (back.length > 0) {
        if (!this.back) {
          this.back = new _CsgNode();
        }
  ...
  function planeFromVertices(a, b, c) {
    const bax = b.x - a.x;
    const bay = b.y - a.y;
    const baz = b.z - a.z;
    const cax = c.x - a.x;
  ...
  function solidFromPolygons(polygons) {
    return { _polygons: polygons };
  }
  function clonePolygons(solid) {
  ...
  function createCsgFromMesh(mesh, materialSlot = 0) {
  ...
  function createMeshFromPolygons(engine, polygons, name) {
    const positions = [];
    const normals = [];
    const uvs = [];
    const indices = [];
    ...
    return createMeshFromData(engine, name, new F32(positions), new F32(normals), new U32(indices), new F32(uvs));
  }
  function createMeshFromCsg(engine, solid, name = "csg") {
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene90.ts
