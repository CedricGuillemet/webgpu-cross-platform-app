...
(() => {
  ...
  function buildRenderTarget(rt, engine) {
    ...
    const { width, height: height2 } = resolveSize(desc);
    rt._width = width;
    rt._height = height2;
  ...
  function resizeSurface(surface) {
    ...
    const scale2 = Math.min(globalThis.devicePixelRatio || 1, surface.maxDevicePixelRatio);
    const w = clientWidth * scale2 | 0;
    const h = clientHeight * scale2 | 0;
  ...
  function applyLightFoOffset(data, scene) {
    ...
    const oz = w[14];
    let count = 0;
    for (const light2 of scene.lights) {
      if (count >= MAX_LIGHTS) {
        break;
      }
      if (!light2._writeLightUbo) {
        continue;
      }
      const o = 4 + count * LIGHT_ENTRY_FLOATS;
      const type = data[o + 3];
      if (type === 0 || type === 2) {
        const lw = light2.worldMatrix;
        data[o] = lw[12] - ox;
        data[o + 1] = lw[13] - oy;
        data[o + 2] = lw[14] - oz;
      }
  ...
  function fillLightsData(data, lights) {
    ...
    for (const light2 of lights) {
      ...
      light2._writeLightUbo(data, headerFloats + count * LIGHT_ENTRY_FLOATS);
      count++;
    }
    _countU32[0] = count;
    data[0] = _countF32[0];
  ...
  function affectsMesh(light2, mesh) {
    const meshId = mesh.id;
    const included = light2.includedOnlyMeshIds;
    if (included?.size) {
      return !!meshId && included.has(meshId);
    }
    return !meshId || !light2.excludedMeshIds?.has(meshId);
  }
...
@fragment fn fs(v:V)->@location(0)vec4f{return textureSample(t,s,v.u);}`;
  ...
  function createDirectionalLight(direction, intensity = 1) {
    ...
    const { wm, onDirty, lvs } = createLightBase(() => {
      return localMatrixFromDirection(light2.direction.x, light2.direction.y, light2.direction.z, light2.position.x, light2.position.y, light2.position.z, _localMatrix);
    });
    const light2 = applyWorldMatrixAccessors(
      {
        ...
        _writeLightUbo: (data, offset) => {
          const o = offset;
          const w = light2.worldMatrix;
          data[o] = w[8];
          data[o + 1] = w[9];
          data[o + 2] = w[10];
          data[o + 3] = 1;
          data[o + 4] = light2.diffuse[0] * light2.intensity;
          data[o + 5] = light2.diffuse[1] * light2.intensity;
          data[o + 6] = light2.diffuse[2] * light2.intensity;
          data[o + 7] = Number.MAX_VALUE;
          data[o + 8] = light2.specular[0] * light2.intensity;
          data[o + 9] = light2.specular[1] * light2.intensity;
          data[o + 10] = light2.specular[2] * light2.intensity;
        }
      },
      wm,
      lvs
  ...
  function computeAabb(positions, world2) {
    let minX = Infinity, minY = Infinity, minZ = Infinity;
    let maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
    if (world2) {
      const m0 = world2[0], m1 = world2[1], m2 = world2[2], m4 = world2[4], m5 = world2[5], m6 = world2[6], m8 = world2[8], m9 = world2[9], m10 = world2[10], m12 = world2[12], m13 = world2[13], m14 = world2[14];
  ...
  async function loadTexture2DImpl(engine, url, opts) {
    ...
    const mipLevelCount = mipMaps ? Math.floor(Math.log2(Math.max(width, height2))) + 1 : 1;
    const texture = device.createTexture({
      size: { width, height: height2 },
      format,
      mipLevelCount,
    ...
    device.queue.copyExternalImageToTexture({ source: imageBitmap, flipY: invertY }, { texture, premultipliedAlpha: premultiplyAlpha }, { width, height: height2 });
    imageBitmap.close();
    ...
    const tex2d = { texture, view: texture.createView(), sampler, width, height: height2 };
  ...
  function parseNodeMaterialSource(source) {
    ...
    if (rawAlpha.forceAlphaBlending === true) {
      ...
      const fragOut = findBlockByClassName({ blocks: blocks2, namedInputs, alphaMode, needsAlphaBlending: false, backFaceCulling: true }, "FragmentOutputBlock");
      const aConn = fragOut?.inputs.get("a");
      needsAlphaBlending = alphaMode > 0 && !!aConn?.source;
    }
    return { blocks: blocks2, namedInputs, alphaMode, needsAlphaBlending, backFaceCulling: rawAlpha.backFaceCulling !== false };
  }
  ...
  function tryResolve(block, inputName, stage, state, ctx) {
    const input2 = block.inputs.get(inputName);
    if (!input2 || !input2.source) {
      return null;
    }
  ...
  var init_scale_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/scale-block.ts"() {
      emitter12 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const input2 = ctx.resolve(block, "input", stage, state);
          const factor = ctx.resolve(block, "factor", stage, state);
          const fc = ctx.cast(factor, "f32").expr;
          return { expr: `(${input2.expr} * ${fc})`, type: input2.type };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/remap-block.ts"() {
      ...
      emitter19 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          return {
            expr: `(${tMin} + (${input2.expr} - ${sMin}) * (${tMax} - ${tMin}) / (${sMax} - ${sMin}))`,
            type: input2.type
          };
        }
      };
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/trigonometry-block.ts"() {
      ...
      emitter20 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          return { expr: `${fn}(${input2.expr})`, type: input2.type };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/transform-block.ts"() {
      ...
      emitter23 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const transform2 = ctx.resolve(block, "transform", stage, state);
          ...
          return { expr: `(${transform2.expr} * ${vec4})`, type: "vec4f" };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/texture-block.ts"() {
      ...
      emitter25 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          let uv2;
          if (uvInput?.source) {
            uv2 = ctx.cast(ctx.resolve(block, "uv", stage, state), "vec2f");
          } else {
            uv2 = { expr: "vec2<f32>(0.0, 0.0)", type: "vec2f" };
          }
          ...
          if (!sampleExpr) {
            ...
            stageState.body.push(`let ${sampleVar} = textureSample(nodeTex_${bindingName3}, nodeSamp_${bindingName3}, ${uv2.expr});`);
...
`;
      emitter31 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const inType = input2.type === "vec4f" ? "vec4f" : "vec3f";
          const inVec3 = ctx.cast(input2, "vec3f").expr;
          const factor = `nme_fogFactor(${wp}, _NME_CAMERA_POS_, _NME_FOG_PARAMS_)`;
          const mixed = `mix(${fogColor}, ${inVec3}, ${factor})`;
          if (inType === "vec4f") {
            return { expr: `vec4<f32>(${mixed}, (${input2.expr}).w)`, type: "vec4f" };
          }
...
`;
      emitter32 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const uv2 = ctx.cast(ctx.resolve(block, "uv", stage, state), "vec2f").expr;
          ...
          return { expr: `nme_perturbNormal(${wp}, ${wn}, ${uv2}, ${nm}, ${strength})`, type: "vec3f" };
        }
      };
    }
  });
...
`;
      emitter33 = {
        ...
        stage: "vertex",
        emit(block, _outputName, stage, state, ctx) {
          const world2 = ctx.resolve(block, "world", stage, state);
          ...
          return { expr: `(${world2.expr} * nme_skinningMatrix(${indices}, ${weights}))`, type: "mat4f" };
        }
      };
    }
  });
  ...
  function fallback(kind) {
    ...
    const zero2 = type === "vec2f" ? "vec2<f32>(0.0)" : type === "vec4f" ? "vec4<f32>(0.0)" : "vec3<f32>(0.0)";
    return { expr: zero2, type };
  }
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/morph-targets.ts"() {
      ...
      emitter35 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const input2 = block.inputs.get(kind);
          if (!input2?.source) {
      ...
      emitter39 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          if (existing) {
            ...
            const ao2 = resolveOptional2(block, "ambientOcc", "1.0", "f32", stage, state, ctx);
            ...
            state.fragment.body.push(
              `let ${callVar} = nme_pbr_mr_compute(${wp}, ${gn}, ${wn}, ${cp}, ${bc}, ${me}, ${ro}, ${ao2}, 0.0, 0.0, 1.5, v3(0.5, 0.5, 1.0), v2(0.0), v3(1.0), 1.0, 0.0, 0.0, v3(1.0), 0.0, ${baseIorExpr}, 0.0, 1.5, 1.0, v3(1.0), 0.0, 0.0, v3(1.0), 0.0, v2(1.0, 0.0), v2(0.0), ${sf});`
            );
            state.fragment.memo.set(memoKey2, { expr: callVar, type: "vec4f" });
          }
          switch (outputName) {
...
`;
      ...
      emitter40 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          if (existing) {
            ...
            const ao2 = resolveOptional3(block, "ambientOcc", "1.0", "f32", stage, state, ctx);
  ...
  var init_reciprocal_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/reciprocal-block.ts"() {
      ...
      emitter49 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const input2 = ctx.resolve(block, "input", stage, state);
          if (input2.type === "mat4f") {
            return { expr: `inverse(${input2.expr})`, type: "mat4f" };
          }
          return { expr: `(1.0 / ${input2.expr})`, type: input2.type };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/reflect-block.ts"() {
      ...
      emitter53 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const normal2 = ctx.cast(ctx.resolve(block, "normal", stage, state), "vec3f").expr;
          return { expr: `reflect(${incident}, ${normal2})`, type: "vec3f" };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/refract-block.ts"() {
      ...
      emitter54 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          return { expr: `refract(${incident}, ${normal2}, ${ior})`, type: "vec3f" };
        }
      };
    }
  });
  ...
  var init_fresnel_block = __esm({
    ...
    return clamp(fresnelTerm, 0.0, 1.0);
      ...
      emitter56 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const worldNormal2 = ctx.cast(ctx.resolve(block, "worldNormal", stage, state), "vec3f").expr;
          ...
          return { expr: `nme_computeFresnelTerm(${viewDirection}, ${worldNormal2}, ${bias}, ${power})`, type: "f32" };
        }
      };
    }
  });
  ...
  function curveExpr(type, input2, inputType) {
    if (inputType === "f32") {
      return curveScalar(type, input2);
    }
    if (inputType === "vec2f") {
      return `${WGSL[inputType]}(${curveScalar(type, `(${input2}).x`)}, ${curveScalar(type, `(${input2}).y`)})`;
    }
    if (inputType === "vec3f") {
      return `${WGSL[inputType]}(${curveScalar(type, `(${input2}).x`)}, ${curveScalar(type, `(${input2}).y`)}, ${curveScalar(type, `(${input2}).z`)})`;
    }
    if (inputType === "vec4f") {
      return `${WGSL[inputType]}(${curveScalar(type, `(${input2}).x`)}, ${curveScalar(type, `(${input2}).y`)}, ${curveScalar(type, `(${input2}).z`)}, ${curveScalar(type, `(${input2}).w`)})`;
    }
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/curve-block.ts"() {
      ...
      emitter60 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          return { expr: curveExpr(curveType, input2.expr, input2.type), type: input2.type };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/wave-block.ts"() {
      ...
      emitter61 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const input2 = ctx.resolve(block, "input", stage, state);
          const v = input2.expr;
          const half = scalar(input2.type, "0.5");
          const one3 = scalar(input2.type, "1.0");
          const two = scalar(input2.type, "2.0");
          const rawKind = block.serialized.kind;
          const kind = typeof rawKind === "number" ? rawKind : 0;
          if (kind === 0) {
            return { expr: `(${v} - floor(${half} + ${v}))`, type: input2.type };
          }
          if (kind === 1) {
            return { expr: `(${one3} - ${two} * round(fract(${v})))`, type: input2.type };
          }
          if (kind === 2) {
            return { expr: `(${two} * abs(${two} * (${v} - floor(${half} + ${v}))) - ${one3})`, type: input2.type };
          }
  ...
  var init_desaturate_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/desaturate-block.ts"() {
      ...
      emitter65 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const color2 = ctx.cast(ctx.resolve(block, "color", stage, state), "vec3f").expr;
          const level = ctx.cast(ctx.resolve(block, "level", stage, state), "f32").expr;
          const minColor = `min(min((${color2}).x, (${color2}).y), (${color2}).z)`;
          const maxColor = `max(max((${color2}).x, (${color2}).y), (${color2}).z)`;
          const merge = `(0.5 * (${minColor} + ${maxColor}))`;
          return { expr: `mix(${color2}, vec3<f32>(${merge}), ${level})`, type: "vec3f" };
        }
      };
    }
  });
  ...
  var init_gradient_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/gradient-block.ts"() {
      ...
      emitter66 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const input2 = block.inputs.get("gradient");
          const steps = readSteps(block.serialized.colorSteps);
          if (!input2?.source || steps.length === 0) {
            return { expr: "vec3<f32>(0.0, 0.0, 0.0)", type: "vec3f" };
          }
          const gradient = ctx.resolve(block, "gradient", stage, state);
  ...
  var init_rotate2d_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/rotate2d-block.ts"() {
      ...
      emitter70 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const input2 = ctx.cast(ctx.resolve(block, "input", stage, state), "vec2f");
          const angle = ctx.cast(ctx.resolve(block, "angle", stage, state), "f32");
          return {
            expr: `vec2<f32>(cos(${angle.expr}) * (${input2.expr}).x - sin(${angle.expr}) * (${input2.expr}).y, sin(${angle.expr}) * (${input2.expr}).x + cos(${angle.expr}) * (${input2.expr}).y)`,
            type: "vec2f"
          };
        }
      };
  ...
  function bindingName(block, inputName, ctx, fallback2) {
    const input2 = block.inputs.get(inputName);
    if (input2?.source) {
      const producer = ctx.graph.blocks.get(input2.source.blockId);
  ...
  function emitTriPlanarSample(block, stage, state, ctx) {
    ...
    const position2 = ctx.cast(ctx.resolve(block, "position", stage, state), "vec3f");
    const normal2 = ctx.cast(ctx.resolve(block, "normal", stage, state), "vec3f");
    const sharpnessInput = block.inputs.get("sharpness")?.source ? ctx.cast(ctx.resolve(block, "sharpness", stage, state), "f32").expr : "1.0";
    const transform2 = textureTransform(block);
    ...
    stageState.body.push(`let ${n} = ${normal2.expr};`);
    stageState.body.push(`var ${uvx} = (${position2.expr}).yz;`);
    stageState.body.push(`var ${uvy} = (${position2.expr}).zx;`);
    stageState.body.push(`var ${uvz} = (${position2.expr}).xy;`);
    ...
    stageState.body.push(`let _cU${temp} = cos(${formatFloat3(transform2.uAng)});`);
    stageState.body.push(`let _sU${temp} = sin(${formatFloat3(transform2.uAng)});`);
    stageState.body.push(`${uvx} = mat2x2<f32>(_cU${temp}, -_sU${temp}, _sU${temp}, _cU${temp}) * ${uvx};`);
    stageState.body.push(`let _cV${temp} = cos(${formatFloat3(transform2.vAng)});`);
    stageState.body.push(`let _sV${temp} = sin(${formatFloat3(transform2.vAng)});`);
    stageState.body.push(`${uvy} = mat2x2<f32>(_cV${temp}, _sV${temp}, -_sV${temp}, _cV${temp}) * ${uvy};`);
    stageState.body.push(`let _cW${temp} = cos(${formatFloat3(transform2.wAng)});`);
    stageState.body.push(`let _sW${temp} = sin(${formatFloat3(transform2.wAng)});`);
    stageState.body.push(`${uvz} = mat2x2<f32>(_cW${temp}, -_sW${temp}, _sW${temp}, _cW${temp}) * ${uvz};`);
    stageState.body.push(`let _scale${temp} = vec2<f32>(${formatFloat3(transform2.uScale)}, ${formatFloat3(transform2.vScale)});`);
    stageState.body.push(`let _offset${temp} = vec2<f32>(${formatFloat3(transform2.uOffset)}, ${formatFloat3(transform2.vOffset)});`);
    ...
    stageState.body.push(
      `let ${sample} = ((${x} * ${w}.x + ${y} * ${w}.y + ${z} * ${w}.z) / (${w}.x + ${w}.y + ${w}.z)) * ${formatFloat3(block.serialized["disableLevelMultiplication"] === true ? 1 : transform2.level)};`
    );
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/triplanar-block.ts"() {
      ...
      emitter71 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          if (outputName === "level") {
            const transform2 = textureTransform(block);
            return { expr: formatFloat3(transform2.level), type: "f32" };
          }
  ...
  function emitBiPlanarSample(block, stage, state, ctx) {
    ...
    const w = `_w${temp}`;
    const sample = `_sample${temp}`;
    stageState.body.push(`let ${p} = ${position2.expr};`);
    stageState.body.push(`let ${dx} = dpdx(${p});`);
    stageState.body.push(`let ${dy} = dpdy(${p});`);
    stageState.body.push(`let ${n} = abs(${normal2.expr});`);
  ...
  function optional(block, inputName, stage, state, ctx) {
    const input2 = block.inputs.get(inputName);
    return input2?.source ? ctx.resolve(block, inputName, stage, state) : null;
  }
...
return sqrt(vec2<f32>(d1, d2));
  ...
  var init_matrix_blocks = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/matrix-blocks.ts"() {
      "use strict";
      SPLIT_OUTPUT = {
        row0: { expr: (input2) => `(${input2})[0]`, type: "vec4f" },
        row1: { expr: (input2) => `(${input2})[1]`, type: "vec4f" },
        row2: { expr: (input2) => `(${input2})[2]`, type: "vec4f" },
        row3: { expr: (input2) => `(${input2})[3]`, type: "vec4f" },
        col0: { expr: (input2) => `vec4<f32>((${input2})[0][0], (${input2})[1][0], (${input2})[2][0], (${input2})[3][0])`, type: "vec4f" },
        col1: { expr: (input2) => `vec4<f32>((${input2})[0][1], (${input2})[1][1], (${input2})[2][1], (${input2})[3][1])`, type: "vec4f" },
        col2: { expr: (input2) => `vec4<f32>((${input2})[0][2], (${input2})[1][2], (${input2})[2][2], (${input2})[3][2])`, type: "vec4f" },
        col3: { expr: (input2) => `vec4<f32>((${input2})[0][3], (${input2})[1][3], (${input2})[2][3], (${input2})[3][3])`, type: "vec4f" }
      };
      emitter77 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const input2 = ctx.cast(ctx.resolve(block, "input", stage, state), "mat4f");
          if (block.className === "MatrixTransposeBlock") {
            return { expr: `transpose(${input2.expr})`, type: "mat4f" };
          }
          if (block.className === "MatrixDeterminantBlock") {
            return { expr: `determinant(${input2.expr})`, type: "f32" };
          }
          if (block.className === "MatrixSplitterBlock") {
            const output2 = SPLIT_OUTPUT[outputName];
            if (!output2) {
              throw new Error(`NodeMaterial: MatrixSplitterBlock has no output "${outputName}"`);
            }
            return { expr: output2.expr(input2.expr), type: output2.type };
          }
  ...
  function emitHeightNormal(block, stage, state, ctx) {
    ...
    const height2 = ctx.cast(ctx.resolve(block, "input", stage, state), "f32").expr;
    const pos = ctx.cast(ctx.resolve(block, "worldPosition", stage, state), "vec3f").expr;
    const normal2 = ctx.cast(ctx.resolve(block, "worldNormal", stage, state), "vec3f").expr;
    ...
    const tangent2 = tangentInput?.source ? ctx.cast(ctx.resolve(block, "worldTangent", stage, state), "vec3f").expr : "vec3<f32>(0.0)";
    const out = `_hn${ctx.temp(state, "heightNormal")}`;
    stageState.body.push(
      `let ${out} = nme_heightToNormal(${height2}, ${pos}, ${tangent2}, ${normal2}, ${boolLiteral(block.serialized.generateInWorldSpace, false)}, ${boolLiteral(block.serialized.automaticNormalizationNormal, true)}, ${boolLiteral(block.serialized.automaticNormalizationTangent, true)});`
    );
...
`;
  ...
  function emitTbnRows(block, stage, state, ctx) {
    ...
    const normal2 = ctx.cast(ctx.resolve(block, "normal", stage, state), "vec3f").expr;
    const tangent2 = ctx.cast(ctx.resolve(block, "tangent", stage, state), "vec4f").expr;
    const world2 = ctx.cast(ctx.resolve(block, "world", stage, state), "mat4f").expr;
    const prefix = `_tbn${ctx.temp(state, "tbn")}`;
    stageState.body.push(`let ${prefix}_normal = normalize(${normal2});`);
    stageState.body.push(`let ${prefix}_tangent = normalize((${tangent2}).xyz);`);
    stageState.body.push(`let ${prefix}_bitangent = cross(${prefix}_normal, ${prefix}_tangent) * (${tangent2}).w;`);
    stageState.body.push(
      `let ${prefix}_mat = mat3x3<f32>((${world2})[0].xyz, (${world2})[1].xyz, (${world2})[2].xyz) * mat3x3<f32>(${prefix}_tangent, ${prefix}_bitangent, ${prefix}_normal);`
    );
...
`;
      emitter82 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const screenSize2 = ctx.cast(ctx.resolve(block, "screenSize", stage, state), "vec2f").expr;
          return {
            expr: `nme_computeAo(nodeTex_${bindingName3}, nodeSamp_${bindingName3}, _NME_FRAG_COORD_, ${screenSize2}, ${num(block.serialized.radius, 1e-4)}, ${num(block.serialized.area, 75e-4)}, ${num(block.serialized.fallOff, 1e-6)})`,
            type: "f32"
          };
        }
      };
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/screen-space-block.ts"() {
      ...
      emitter85 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const wvp2 = ctx.cast(ctx.resolve(block, "worldViewProjection", stage, state), "mat4f");
          const clip = ctx.temp(state, "screenClip");
          const uv2 = ctx.temp(state, "screenUv");
          const vectorExpr = vector.type === "vec4f" ? vector.expr : `vec4<f32>(${vector.expr}, 1.0)`;
          state.fragment.body.push(`let ${clip} = ${wvp2.expr} * ${vectorExpr};`);
          state.fragment.body.push(`let ${uv2} = (${clip}.xy / ${clip}.w) * 0.5 + vec2<f32>(0.5, 0.5);`);
          const out = OUTPUTS4[outputName];
          if (!out) {
            throw new Error(`NodeMaterial: ScreenSpaceBlock has no output "${outputName}"`);
          }
          return { expr: `${uv2}${out.swizzle}`, type: out.type };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/twirl-block.ts"() {
      ...
      emitter86 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          state.fragment.body.push(`let ${delta} = ${input2.expr} - ${center.expr};`);
      ...
      emitter87 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          if (depthConn?.source) {
            ...
            const worldPos2 = ctx.cast(ctx.resolve(block, "worldPos", stage, state), "vec4f");
            const viewProjection = ctx.cast(ctx.resolve(block, "viewProjection", stage, state), "mat4f");
            const p = ctx.temp(state, "fragDepthP");
            state.fragment.body.push(`let ${p} = ${viewProjection.expr} * ${worldPos2.expr};`);
  ...
  var init_debug_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/debug-block.ts"() {
      ...
      emitter91 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const input2 = block.inputs.get("debug");
          if (!input2?.source) {
            return { expr: "0.0", type: "f32" };
          }
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/mesh-attribute-exists-block.ts"() {
      ...
      emitter92 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const fallback2 = ctx.cast(ctx.resolve(block, "fallback", stage, state), input2.type);
          const expr = ctx.temp(state, "attr");
          const s = stage === "vertex" ? state.vertex : state.fragment;
          s.body.push(`let ${expr} = select(${fallback2.expr}, ${input2.expr}, ${flag} > 0.5);`);
          return { expr, type: input2.type };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/image-processing-block.ts"() {
      ...
      emitter95 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const color2 = ctx.cast(ctx.resolve(block, "color", stage, state), "vec4f");
          const t = ctx.temp(state, "ip");
          state.fragment.body.push(`let ${t} = nme_apply_image_processing(${color2.expr});`);
          if (outputName === "rgb") {
  ...
  function makeContext(graph, loadedEmitters) {
    const ctx = {
      ...
      resolve: (block, inputName, stage, state) => {
        ...
        const producer = graph.blocks.get(input2.source.blockId);
        if (!producer) {
          throw new Error(`NodeMaterial: dangling connection ${block.id}.${inputName} -> block ${input2.source.blockId}`);
        }
        return ctx.resolveOutput(producer, input2.source.outputName, stage, state);
      },
...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene83-nme.ts
  var TARGET_VERTEX = 1;
  var TARGET_FRAGMENT = 2;
  var TARGET_VERTEX_AND_FRAGMENT = 3;
  var TARGET_NEUTRAL = 4;
  var TYPE_FLOAT = 1;
  var TYPE_VECTOR2 = 4;
  var TYPE_VECTOR3 = 8;
  var TYPE_VECTOR4 = 16;
  ...
  function addBlock(className, name, target, inputs, outputs, extra = {}) {
    const id = nextId++;
    blocks.push({
  ...
  function inputBlock(name, type, value, mode = 0, systemValue = null, target = mode === 1 ? TARGET_VERTEX : TARGET_NEUTRAL) {
    const extra = {
      type,
      mode,
      systemValue,
  ...
  function transform(name, vector, matrix, complementW) {
  ...
  function split(name, source, sourceOutput, inputName) {
  ...
  function scale(name, source, sourceOutput, factor) {
  ...
  function add(name, left, leftOutput, right, rightOutput) {
  ...
  function multiply(name, left, leftOutput, right, rightOutput) {
  ...
  function vector3(name, xy, xyOutput, z) {
    return addBlock(
      "VectorMergerBlock",
      name,
      TARGET_NEUTRAL,
  ...
  var position = inputBlock("position", TYPE_VECTOR3, null, 1);
  var normal = inputBlock("normal", TYPE_VECTOR3, null, 1);
  var uv = inputBlock("uv", TYPE_VECTOR2, null, 1);
  inputBlock("position", TYPE_VECTOR3, null, 1);
  var world = inputBlock("world", TYPE_MATRIX, null, 0, 1, TARGET_VERTEX);
  var wvp = inputBlock("worldViewProjection", TYPE_MATRIX, null, 0, 6, TARGET_VERTEX);
  var cameraPosition = inputBlock("cameraPosition", TYPE_VECTOR3, null, 0, 7, TARGET_FRAGMENT);
  var clipPos = transform("TransformWVP", position, wvp, 1);
  var worldPos = transform("TransformWorldPos", position, world, 1);
  var worldNormal = transform("TransformWorldNormal", normal, world, 0);
  split("SplitWorldPos", worldPos, "output", "xyzw");
  split("SplitWorldNormal", worldNormal, "output", "xyzw");
  var vertexOutput = addBlock("VertexOutputBlock", "VertexOutput", TARGET_VERTEX, [input("vector", { id: clipPos, output: "output" })], []);
  var uvSplit = split("SplitUV", uv, "output", "xy");
  var zero = inputBlock("zero", TYPE_FLOAT, 0);
  var uvPosition = vector3("UvWorldPosition", uv, "output", zero);
  ...
  var freqX = inputBlock("heightFreqX", TYPE_FLOAT, 18);
  var freqY = inputBlock("heightFreqY", TYPE_FLOAT, 15);
  var heightAmp = inputBlock("heightAmp", TYPE_FLOAT, 0.075);
  var xPhase = scale("HeightXPhase", uvSplit, "x", freqX);
  var yPhase = scale("HeightYPhase", uvSplit, "y", freqY);
  var sinX = trig("HeightSinX", xPhase, "output", 1);
  var cosY = trig("HeightCosY", yPhase, "output", 0);
  var heightSum = add("HeightSum", sinX, "output", cosY, "output");
  var height = scale("Height", heightSum, "output", heightAmp);
  var tangent = inputBlock("worldTangentConst", TYPE_VECTOR4, [1, 0, 0, 1], 0, null, TARGET_FRAGMENT);
  var normalConst = inputBlock("worldNormalConst", TYPE_VECTOR3, [0, 0, -1], 0, null, TARGET_FRAGMENT);
  var h2n = addBlock(
    "HeightToNormalBlock",
    "HeightToNormal",
    TARGET_FRAGMENT,
  ...
  var deriv = addBlock("DerivativeBlock", "DerivativeHeight", TARGET_FRAGMENT, [input("input", { id: height, output: "output" })], [output("dx"), output("dy")]);
  var normalHalf = inputBlock("normalHalf", TYPE_FLOAT, 0.5);
  var derivScaleK = inputBlock("derivativeNormalScale", TYPE_FLOAT, 12);
  var one2 = inputBlock("one", TYPE_FLOAT, 1);
  var dxScaled = scale("DerivativeDxScale", deriv, "dx", derivScaleK);
  var dyScaled = scale("DerivativeDyScale", deriv, "dy", derivScaleK);
  var dxColor = add("DerivativeDxColor", normalHalf, "output", dxScaled, "output");
  var dyColor = add("DerivativeDyColor", normalHalf, "output", dyScaled, "output");
  var derivativeNormalColor = color("DerivativeNormalColor", dxColor, "output", dyColor, "output", one2, "output");
  var normalBlend = addBlock("NormalBlendBlock", "NormalBlend", TARGET_NEUTRAL, [input("normalMap0", { id: h2n, output: "xyz" }), input("normalMap1", { id: derivativeNormalColor, output: "rgb" })], [output("output")]);
  var perturbStrength = inputBlock("perturbStrength", TYPE_FLOAT, 0.75);
  var tbn = addBlock("TBNBlock", "TBN", TARGET_FRAGMENT, [input("normal", { id: normalConst, output: "output" }), input("tangent", { id: tangent, output: "output" }), input("world", { id: world, output: "output" })], [output("TBN"), output("row0"), output("row1"), output("row2")]);
  var perturb = addBlock(
    "PerturbNormalBlock",
    "PerturbNormal",
    TARGET_NEUTRAL,
    [
  ...
  var tbnAbs = trig("TbnRow2Abs", tbn, "row2", 2);
  var tbnTintScale = inputBlock("tbnTintScale", TYPE_FLOAT, 0.08);
  var tbnTint = scale("TbnTint", tbnAbs, "output", tbnTintScale);
  var baseColor = inputBlock("baseColor", TYPE_COLOR3, [0.82, 0.56, 0.28]);
  var litColor = add("LitColorWithTbnTint", baseColor, "output", tbnTint, "output");
  var light = addBlock(
    "LightBlock",
    "Light",
    TARGET_FRAGMENT,
  ...
  var aoSource = addBlock("ImageSourceBlock", "AoDepth", TARGET_VERTEX_AND_FRAGMENT, [], [output("source"), output("dimensions")]);
  var screenSize = inputBlock("screenSize", TYPE_VECTOR2, [1280, 720]);
  var ao = addBlock("AmbientOcclusionBlock", "AmbientOcclusion", TARGET_FRAGMENT, [input("source", { id: aoSource, output: "source" }), input("screenSize", { id: screenSize, output: "output" })], [output("occlusion")], {
    radius: 1e-4,
    area: 0.1,
    fallOff: -0.1
  });
  var finalColor = multiply("ApplyAO", light, "diffuseOutput", ao, "occlusion");
  var fragmentOutput = addBlock("FragmentOutputBlock", "FragmentOutput", TARGET_FRAGMENT, [input("rgba"), input("rgb", { id: finalColor, output: "output" }), input("a")], [], {
    convertToGammaSpace: false,
    convertToLinearSpace: false,
    useLogarithmicDepth: false
  });
  var SCENE83_NME_JSON = {
    tags: null,
    ignoreAlpha: false,
  ...
  var SCENE83_POSITION_TEXTURE_URL = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAYAAABytg0kAAAAF0lEQVR4AWNkYGj4/5+BgYGF4T8DGAAAKaYDg36vB4oAAAAASUVORK5CYII=";

  // ../../../Babylon-Lite/lab/lite/src/lite/scene83.ts
  async function loadScene83BlockEmitter(className) {
    switch (className) {
      case "AddBlock":
        return (await Promise.resolve().then(() => (init_add_block(), add_block_exports))).emitter;
      case "AmbientOcclusionBlock":
        return (await Promise.resolve().then(() => (init_ambient_occlusion_block(), ambient_occlusion_block_exports))).emitter;
      case "ColorMergerBlock":
        return (await Promise.resolve().then(() => (init_color_merger(), color_merger_exports))).emitter;
      case "DerivativeBlock":
        return (await Promise.resolve().then(() => (init_derivative_block(), derivative_block_exports))).emitter;
      case "FragmentOutputBlock":
        return (await Promise.resolve().then(() => (init_fragment_output(), fragment_output_exports))).emitter;
      case "HeightToNormalBlock":
        return (await Promise.resolve().then(() => (init_height_to_normal_block(), height_to_normal_block_exports))).emitter;
      case "ImageSourceBlock":
        return (await Promise.resolve().then(() => (init_image_source(), image_source_exports))).emitter;
      case "InputBlock":
        return (await Promise.resolve().then(() => (init_input_block(), input_block_exports))).emitter;
      case "LightBlock":
        return (await Promise.resolve().then(() => (init_light_block(), light_block_exports))).emitter;
      case "MultiplyBlock":
        return (await Promise.resolve().then(() => (init_multiply_block(), multiply_block_exports))).emitter;
      case "NormalBlendBlock":
        return (await Promise.resolve().then(() => (init_normal_blend_block(), normal_blend_block_exports))).emitter;
      case "PerturbNormalBlock":
        return (await Promise.resolve().then(() => (init_perturb_normal(), perturb_normal_exports))).emitter;
      case "ScaleBlock":
        return (await Promise.resolve().then(() => (init_scale_block(), scale_block_exports))).emitter;
      case "TBNBlock":
        return (await Promise.resolve().then(() => (init_tbn_block(), tbn_block_exports))).emitter;
      case "TextureBlock":
      ...
      case "TrigonometryBlock":
        return (await Promise.resolve().then(() => (init_trigonometry_block(), trigonometry_block_exports))).emitter;
      case "VectorMergerBlock":
        return (await Promise.resolve().then(() => (init_vector_merger(), vector_merger_exports))).emitter;
      case "VectorSplitterBlock":
        return (await Promise.resolve().then(() => (init_vector_splitter(), vector_splitter_exports))).emitter;
      case "VertexOutputBlock":
