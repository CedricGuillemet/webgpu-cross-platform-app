...
${varyBody}
  ...
  function getOrCreateShaderPipeline(engine, sig, material, bindings, variantKey = "", vertexBuffers = bindings.vertexBuffers, instanceAttrs = "") {
    ...
    const stencil = material.stencil && _stencilResolver2 ? _stencilResolver2(material.stencil) : null;
    const device = engine._device;
...
`;
  ...
  var _stencilResolver2, SHADER_STAGE_ALL;
...
`;
  ...
  function setShaderUniform(material, name, value) {
...
`
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/compute-normals.ts
  init_typed_arrays();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/create-cylinder.ts
  init_typed_arrays();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/create-ribbon.ts
  init_typed_arrays();
  function len(v) {
  ...
  function createRibbonData(options) {
    ...
    let idc = 0;
    for (let p2 = 0; p2 < pathArray.length + closeArrayCorr; p2++) {
      uTotalDistance[p2] = 0;
      us[p2] = [0];
      const path = p2 === pathArray.length ? pathArray[0] : pathArray[p2];
      const l = path.length;
      ...
      while (j < l) {
        ...
        if (j > 0) {
          ...
          const dist = vectlg + uTotalDistance[p2];
          us[p2].push(dist);
          uTotalDistance[p2] = dist;
        }
        j++;
      }
      if (closePath) {
    ...
    for (let i = 0; i < minlg + closePathCorr; i++) {
      vTotalDistance[i] = 0;
      vs[i] = [0];
      for (let p2 = 0; p2 < pathArray.length - 1 + closeArrayCorr; p2++) {
        const path1 = pathArray[p2];
        const path2 = p2 === pathArray.length - 1 ? pathArray[0] : pathArray[p2 + 1];
        let v1;
        let v2;
        if (i === minlg) {
          v1 = path1[0];
    ...
    for (let p2 = 0; p2 < pathArray.length + closeArrayCorr; p2++) {
      for (let i = 0; i < minlg + closePathCorr; i++) {
        const u = uTotalDistance[p2] !== 0 ? us[p2][i] / uTotalDistance[p2] : 0;
        const v = vTotalDistance[i] !== 0 ? vs[i][p2] / vTotalDistance[i] : 0;
        uvs.push(u, v);
      }
    }
    let p = 0;
    ...
    if (closePath) {
      for (let p2 = 0; p2 < pathArray.length; p2++) {
        const indexFirst = idx[p2] * 3;
        const indexLast = p2 + 1 < pathArray.length ? (idx[p2 + 1] - 1) * 3 : normals.length - 3;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/math/cross-vec3.ts
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/math/length-vec3.ts
  function lengthVec3(v) {
    return Math.sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
  }

  // ../../../Babylon-Lite/packages/babylon-lite/src/math/normalize-vec3-object.ts
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/math/sub-vec3.ts
  function subVec3(a, b) {
    return { x: a.x - b.x, y: a.y - b.y, z: a.z - b.z };
  }

  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/path3d.ts
  var EPSILON = 1e-3;
  function withinEpsilon(a, b, eps) {
    return Math.abs(a - b) <= eps;
  }
  function getFirstNonNullVector(curve, index) {
    let i = 1;
  ...
  function getLastNonNullVector(curve, index) {
    let i = 1;
  ...
  function computePath3D(curve, firstNormal = null) {
    ...
    const tangents = new Array(l);
    const normals = new Array(l);
    const binormals = new Array(l);
    const distances = new Array(l);
    if (l < 2) {
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/mesh/create-tube.ts
  var CAP_NONE = 0;
  var CAP_START = 1;
  var CAP_END = 2;
  var CAP_ALL = 3;
  ...
  function createTubeData(options) {
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/math/dot-vec3.ts
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/gizmo/utility-layer.ts
  init_scene_core();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/gizmo/pointer-drag.ts
  init_camera();

  // ../../../Babylon-Lite/packages/babylon-lite/src/gizmo/gizmo-math.ts
  var normalizeVec3Obj = normalizeVec3;
  ...
  function quatNormalize(q) {
    const len2 = Math.hypot(q[0], q[1], q[2], q[3]) || 1;
    return [q[0] / len2, q[1] / len2, q[2] / len2, q[3] / len2];
  }
  ...
  function lookAtQuat(dir) {
    const len2 = lengthVec3(dir);
    if (len2 < 1e-8) {
      return [0, 0, 0, 1];
    }
    const fx = dir.x / len2, fy = dir.y / len2, fz = dir.z / len2;
    const dot = fz;
    if (dot > 0.9999999) {
      return [0, 0, 0, 1];
    }
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/gizmo/gizmo-core.ts
  init_create_standard_material();
  init_scene_core();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/gizmo/plane-rotation-gizmo.ts
  init_scene_core();
  init_shader_material();
  init_mat4_invert();

  // ../../../Babylon-Lite/packages/babylon-lite/src/gizmo/rotation-sector-material.ts
  init_shader_material();
  var VERTEX_SOURCE = `struct VertexOutput{@builtin(position) position:vec4<f32>,@location(0) uv:vec2<f32>,};
@vertex fn mainVertex(input:VertexInput)->VertexOutput{var out:VertexOutput;out.position=shaderSystem.worldViewProjection*vec4<f32>(input.position,1.0);out.uv=input.uv;return out;}`;
  var FRAGMENT_SOURCE = `struct VertexOutput{@builtin(position) position:vec4<f32>,@location(0) uv:vec2<f32>,};
...
return vec4<f32>(shaderUniforms.rotationColor,min(intensity*0.25,0.8))*opacity;
  ...
  function createRotationSectorMaterial(initialColor) {
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/gizmo/composite-gizmos.ts
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/physics/havok-queries.ts
  var IGNORE_NONE = [BigInt(0)];
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene49.ts
  var CAPTURE_STEPS = 5;
  var GROUP_Y = 2.5;
  var CAM_DIR = { x: 0, y: 0.5, z: -0.866 };
  var PROX_NUDGE = 0.08;
  var CAST_NUDGE = 0.2;
  ...
  async function main() {
    ...
    function buildGroup(yOffset) {
      ...
      const body = createPhysicsBody(world, capsule, 1 /* ANIMATED */);
