...
(() => {
  ...
  function executePassBody(task, pass) {
    ...
    if (v) {
      const rw = rt._width;
      const rh = rt._height;
      const x2 = Math.floor(v.x * rw);
      const y2 = Math.floor((1 - v.y - v.height) * rh);
      const w = Math.ceil((v.x + v.width) * rw) - x2;
      const h = Math.ceil((1 - v.y) * rh) - y2;
      pass.setViewport(x2, y2, w, h, 0, 1);
      pass.setScissorRect(x2, y2, w, h);
    }
  ...
  function mat4LookAtLH(eye, target, up2) {
    ...
    zAxis.z *= invZ;
    const xAxis = {
      x: up2.y * zAxis.z - up2.z * zAxis.y,
      y: up2.z * zAxis.x - up2.x * zAxis.z,
      z: up2.x * zAxis.y - up2.y * zAxis.x
    };
  ...
  function createEulerProxy(rq) {
    ...
    const apply = (x2, y2, z) => {
      ...
      ey = y2;
      ez = z;
      const [a, b, c, d] = eulerToQuat(x2, y2, z);
      rq.set(a, b, c, d);
      syncedVersion = rq.version;
    };
    return {
  ...
  function computeAabb(positions, world) {
    ...
    if (world) {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        const x2 = m0 * lx + m4 * ly + m8 * lz + m12;
        const y2 = m1 * lx + m5 * ly + m9 * lz + m13;
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/vector-merger.ts"() {
      ...
      emitter2 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const x2 = tryResolve(block, "x", stage, state, ctx);
          const y2 = tryResolve(block, "y", stage, state, ctx);
          const z = tryResolve(block, "z", stage, state, ctx);
          const w = tryResolve(block, "w", stage, state, ctx);
          const sx = x2 ? ctx.cast(x2, "f32").expr : xyIn ? `(${xyIn.expr}).x` : xyzIn ? `(${xyzIn.expr}).x` : "0.0";
          const sy = y2 ? ctx.cast(y2, "f32").expr : xyIn ? `(${xyIn.expr}).y` : xyzIn ? `(${xyzIn.expr}).y` : "0.0";
  ...
  function applyColorSpace(expr, outputName, convertToLinear, convertToGamma) {
    ...
    const power2 = convertToLinear ? "2.2" : "0.45454545";
    if (outputName === "rgba") {
      return `vec4<f32>(pow(max(${expr}.xyz, vec3<f32>(0.0)), vec3<f32>(${power2})), ${expr}.w)`;
    }
    if (outputName === "rgb") {
      return `pow(max(${expr}.xyz, vec3<f32>(0.0)), vec3<f32>(${power2}))`;
    }
    if (outputName === "r" || outputName === "g" || outputName === "b") {
      return `pow(max(${expr}${OUTPUT[outputName].swizzle}, 0.0), ${power2})`;
    }
    return expr;
  }
...
`;
  ...
  var init_reflect_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/reflect-block.ts"() {
      ...
      emitter53 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const incident2 = ctx.cast(ctx.resolve(block, "incident", stage, state), "vec3f").expr;
          const normal2 = ctx.cast(ctx.resolve(block, "normal", stage, state), "vec3f").expr;
          return { expr: `reflect(${incident2}, ${normal2})`, type: "vec3f" };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/refract-block.ts"() {
      ...
      emitter54 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const incident2 = ctx.cast(ctx.resolve(block, "incident", stage, state), "vec3f").expr;
          const normal2 = ctx.cast(ctx.resolve(block, "normal", stage, state), "vec3f").expr;
          const ior2 = ctx.cast(ctx.resolve(block, "ior", stage, state), "f32").expr;
          return { expr: `refract(${incident2}, ${normal2}, ${ior2})`, type: "vec3f" };
        }
      };
    }
  });
  ...
  var init_arc_tan2_block = __esm({
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/arc-tan2-block.ts"() {
      ...
      emitter55 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const x2 = ctx.cast(ctx.resolve(block, "x", stage, state), "f32").expr;
          const y2 = ctx.cast(ctx.resolve(block, "y", stage, state), "f32").expr;
          return { expr: `atan2(${x2}, ${y2})`, type: "f32" };
        }
      };
    }
  });
    ...
    return clamp(fresnelTerm, 0.0, 1.0);
      ...
      emitter56 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          const viewDirection2 = ctx.cast(ctx.resolve(block, "viewDirection", stage, state), "vec3f").expr;
          const bias2 = ctx.cast(ctx.resolve(block, "bias", stage, state), "f32").expr;
          const power2 = ctx.cast(ctx.resolve(block, "power", stage, state), "f32").expr;
          const stageState = stage === "vertex" ? state.vertex : state.fragment;
          stageState.helpers.set("nme_computeFresnelTerm", HELPER);
          return { expr: `nme_computeFresnelTerm(${viewDirection2}, ${worldNormal}, ${bias2}, ${power2})`, type: "f32" };
        }
      };
    }
  });
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/wave-block.ts"() {
      ...
      emitter61 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          const input2 = ctx.resolve(block, "input", stage, state);
          const v = input2.expr;
          const half2 = scalar(input2.type, "0.5");
          const one3 = scalar(input2.type, "1.0");
          const two2 = scalar(input2.type, "2.0");
          const rawKind = block.serialized.kind;
          const kind = typeof rawKind === "number" ? rawKind : 0;
          if (kind === 0) {
            return { expr: `(${v} - floor(${half2} + ${v}))`, type: input2.type };
          }
          if (kind === 1) {
            return { expr: `(${one3} - ${two2} * round(fract(${v})))`, type: input2.type };
          }
          if (kind === 2) {
            return { expr: `(${two2} * abs(${two2} * (${v} - floor(${half2} + ${v}))) - ${one3})`, type: input2.type };
          }
  ...
  function applyColorSpace2(expr, outputName, convertToLinear, convertToGamma) {
    ...
    if (outputName === "r" || outputName === "g" || outputName === "b") {
      return `pow(max(${expr}${OUTPUT2[outputName].swizzle}, 0.0), ${power2})`;
    }
    return expr;
  }
  function emitTriPlanarSample(block, stage, state, ctx) {
    ...
    stageState.body.push(`let ${x2} = textureSample(nodeTex_${texX}, nodeSamp_${texX}, ${uvx});`);
    stageState.body.push(`let ${y2} = textureSample(nodeTex_${texY}, nodeSamp_${texY}, ${uvy});`);
    ...
    stageState.body.push(
      `let ${sample} = ((${x2} * ${w}.x + ${y2} * ${w}.y + ${z} * ${w}.z) / (${w}.x + ${w}.y + ${w}.z)) * ${formatFloat3(block.serialized["disableLevelMultiplication"] === true ? 1 : transform2.level)};`
    );
  ...
  function applyColorSpace3(expr, outputName, convertToLinear, convertToGamma) {
    ...
    if (outputName === "r" || outputName === "g" || outputName === "b") {
      return `pow(max(${expr}${OUTPUT3[outputName].swizzle}, 0.0), ${power2})`;
    }
    return expr;
  }
  function emitBiPlanarSample(block, stage, state, ctx) {
    ...
    stageState.body.push(
      `let ${x2} = textureSampleGrad(nodeTex_${texX}, nodeSamp_${texX}, vec2<f32>(${p}[${ma}.y], ${p}[${ma}.z]), vec2<f32>(${dx}[${ma}.y], ${dx}[${ma}.z]), vec2<f32>(${dy}[${ma}.y], ${dy}[${ma}.z]));`
    );
    stageState.body.push(
      `let ${y2} = textureSampleGrad(nodeTex_${texY}, nodeSamp_${texY}, vec2<f32>(${p}[${me}.y], ${p}[${me}.z]), vec2<f32>(${dx}[${me}.y], ${dx}[${me}.z]), vec2<f32>(${dy}[${me}.y], ${dy}[${me}.z]));`
    );
    ...
    stageState.body.push(`let ${sample} = (${x2} * ${w}.x + ${y2} * ${w}.y) / (${w}.x + ${w}.y);`);
    const result = { expr: sample, type: "vec4f" };
    stageState.memo.set(memoKey2, result);
    return result;
  }
...
return sqrt(vec2<f32>(d1, d2));
  ...
  function emitHeightNormal(block, stage, state, ctx) {
    ...
    stageState.body.push(
      `let ${out} = nme_heightToNormal(${height}, ${pos}, ${tangent}, ${normal2}, ${boolLiteral(block.serialized.generateInWorldSpace, false)}, ${boolLiteral(block.serialized.automaticNormalizationNormal, true)}, ${boolLiteral(block.serialized.automaticNormalizationTangent, true)});`
    );
    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/twirl-block.ts"() {
      ...
      emitter86 = {
        ...
        emit(block, outputName, stage, state, ctx) {
          ...
          const center2 = ctx.cast(block.inputs.get("center")?.source ? ctx.resolve(block, "center", stage, state) : defaultVec2("vec2<f32>(0.5, 0.5)"), "vec2f");
          ...
          state.fragment.body.push(`let ${delta} = ${input2.expr} - ${center2.expr};`);
          state.fragment.body.push(`let ${angle} = ${strength.expr} * length(${delta});`);
          state.fragment.body.push(
            `let ${result} = vec2<f32>(cos(${angle}) * ${delta}.x - sin(${angle}) * ${delta}.y, sin(${angle}) * ${delta}.x + cos(${angle}) * ${delta}.y) + ${center2.expr} + ${offset.expr};`
          );
...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene78-nme.ts
  var TARGET_VERTEX = 1;
  ...
  function unary(className, name, inputName, source, sourceOutput = "output") {
    return addBlock(className, name, TARGET_NEUTRAL, [input(inputName, { id: source, output: sourceOutput })], [output("output")]);
  }
  function binary(className, name, left, right, leftOutput = "output", rightOutput = "output") {
    return addBlock(
      className,
      name,
      TARGET_NEUTRAL,
  ...
  function merge3(name, x2, y2, z, xOutput = "output", yOutput = "output", zOutput = "output") {
    return addBlock(
      "VectorMergerBlock",
      name,
      TARGET_NEUTRAL,
      [
        ...
        input("xy "),
        input("zw "),
        input("x", { id: x2, output: xOutput }),
        input("y", { id: y2, output: yOutput }),
  ...
  var vertexOutput = addBlock("VertexOutputBlock", "VertexOutput", TARGET_VERTEX, [input("vector", { id: transform, output: "output" })], []);
  var uv = inputBlock("uv", TYPE_VECTOR2, null, 1);
  var uvSplit = split("SplitUV", uv, "output", "xy");
  var x = { id: uvSplit, output: "x" };
  var y = { id: uvSplit, output: "y" };
  var zero = inputBlock("zero", TYPE_FLOAT, 0);
  var half = inputBlock("half", TYPE_FLOAT, 0.5);
  var one2 = inputBlock("one", TYPE_FLOAT, 1);
  var two = inputBlock("two", TYPE_FLOAT, 2);
  var three = inputBlock("three", TYPE_FLOAT, 3);
  var four = inputBlock("four", TYPE_FLOAT, 4);
  var minusOne = inputBlock("minusOne", TYPE_FLOAT, -1);
  var twoPi = inputBlock("twoPi", TYPE_FLOAT, 6.283185307179586);
  var ior = inputBlock("ior", TYPE_FLOAT, 0.66);
  var bias = inputBlock("bias", TYPE_FLOAT, 0);
  var power = inputBlock("power", TYPE_FLOAT, 2);
  var center = inputBlock("center", TYPE_VECTOR3, [0.25, 0.75, 0.5]);
  var up = inputBlock("up", TYPE_VECTOR3, [0, 1, 0]);
  var normalBase = inputBlock("normalBase", TYPE_VECTOR3, [0.2, 0.4, 1]);
  var fresnelNormal = inputBlock("fresnelNormal", TYPE_VECTOR4, [0.2, 0.4, 1, 0]);
  var xTimesFour = binary("MultiplyBlock", "xTimesFour", uvSplit, four, "x");
  var modStripe = binary("ModBlock", "ModStripe", xTimesFour, one2);
  var xDivTwo = binary("DivideBlock", "DivideXByTwo", uvSplit, two, "x");
  var xPlusOne = binary("AddBlock", "XPlusOne", uvSplit, one2, "x");
  var reciprocal = unary("ReciprocalBlock", "ReciprocalOneOverXPlusOne", "input", xPlusOne);
  var xMinusHalf = binary("SubtractBlock", "XMinusHalf", uvSplit, half, "x");
  var yMinusHalf = binary("SubtractBlock", "YMinusHalf", uvSplit, half, "y");
  var atan2 = addBlock("ArcTan2Block", "ArcTan2Center", TARGET_NEUTRAL, [input("x", x), input("y", y)], [output("output")]);
  var atanScaled = binary("DivideBlock", "ArcTan2Normalize", atan2, twoPi);
  var atanMapped = binary("AddBlock", "ArcTan2Map", atanScaled, half);
  var rSumA = binary("AddBlock", "RAddDivideMod", xDivTwo, modStripe);
  var rSumB = binary("AddBlock", "RAddReciprocal", rSumA, reciprocal);
  var rSumC = binary("AddBlock", "RAddArcTan2", rSumB, atanMapped);
  var red = binary("DivideBlock", "RedAverage", rSumC, four);
  var uvVec3 = merge3("UvVector3", xMinusHalf, yMinusHalf, half);
  var lengthValue = unary("LengthBlock", "LengthUvVector", "value", uvVec3, "xyz");
  var uvPosition = merge3("UvPosition", uvSplit, uvSplit, zero, "x", "y");
  var distanceValue = binary("DistanceBlock", "DistanceToCenter", uvPosition, center, "xyz");
  var normalizedUvVector = unary("NormalizeBlock", "NormalizeUvVector", "input", uvVec3, "xyz");
  var crossVector = binary("CrossBlock", "CrossWithUp", normalizedUvVector, up);
  var crossSplit = split("SplitCross", crossVector, "output", "xyz");
  var crossHalf = binary("MultiplyBlock", "CrossZHalf", crossSplit, half, "z");
  var crossMapped = binary("AddBlock", "CrossZMap", crossHalf, half);
  var gSumA = binary("AddBlock", "GAddLengthDistance", lengthValue, distanceValue);
  var gSumB = binary("AddBlock", "GAddCross", gSumA, crossMapped);
  var green = binary("DivideBlock", "GreenAverage", gSumB, three);
  var incidentRaw = merge3("IncidentRaw", xMinusHalf, yMinusHalf, minusOne);
  var incident = unary("NormalizeBlock", "NormalizeIncident", "input", incidentRaw, "xyz");
  var normal = unary("NormalizeBlock", "NormalizeNormal", "input", normalBase);
  var reflectVector = addBlock("ReflectBlock", "ReflectIncident", TARGET_NEUTRAL, [input("incident", { id: incident, output: "output" }), input("normal", { id: normal, output: "output" })], [output("output")]);
  var reflectSplit = split("SplitReflect", reflectVector, "output", "xyz");
  var reflectHalf = binary("MultiplyBlock", "ReflectZHalf", reflectSplit, half, "z");
  var reflectMapped = binary("AddBlock", "ReflectZMap", reflectHalf, half);
  var refractVector = addBlock(
    "RefractBlock",
    "RefractIncident",
    TARGET_NEUTRAL,
  ...
  var refractSplit = split("SplitRefract", refractVector, "output", "xyz");
  var refractHalf = binary("MultiplyBlock", "RefractZHalf", refractSplit, half, "z");
  var refractMapped = binary("AddBlock", "RefractZMap", refractHalf, half);
  var viewX = binary("SubtractBlock", "ViewX", half, uvSplit, "output", "x");
  var viewY = binary("SubtractBlock", "ViewY", half, uvSplit, "output", "y");
  var viewRaw = merge3("ViewRaw", viewX, viewY, one2);
  var viewDirection = unary("NormalizeBlock", "NormalizeViewDirection", "input", viewRaw, "xyz");
  var fresnel = addBlock(
    "FresnelBlock",
    "Fresnel",
    TARGET_NEUTRAL,
  ...
  var bSumA = binary("AddBlock", "BAddReflectRefract", reflectMapped, refractMapped);
  var bSumB = binary("AddBlock", "BAddFresnel", bSumA, fresnel, "output", "fresnel");
  var blue = binary("DivideBlock", "BlueAverage", bSumB, three);
  var color = addBlock(
    "VectorMergerBlock",
    "MergeMathColor",
    TARGET_FRAGMENT,
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene78.ts
  async function main() {
