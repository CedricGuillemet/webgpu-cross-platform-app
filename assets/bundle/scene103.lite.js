...
${varyBody}
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene103.ts
  var PHYSICS_FPS = 60;
  ...
  var WALL_D = 3;
  var BLOCK_SIZE = 2;
  var TOTAL = WALL_W * WALL_H * WALL_D;
  var CAM_ALPHA = Math.PI / 2;
  var CAM_BETA = Math.PI / 2.4;
  var CAM_RADIUS = 38;
  var CAM_TARGET = { x: -1, y: 5, z: -1 };
  var RAY_TARGETS = [
    [5, 3, 2],
    [2, 1, 2],
    [8, 5, 2],
    ...
    for (let i = 0; i < WALL_W; i++) {
      for (let j = 0; j < WALL_H; j++) {
        for (let k = 0; k < WALL_D; k++) {
          ...
          const shape = createPhysicsShape(world, { type: 3 /* BOX */, parameters: { extents: blockExtents } });
          const body = createPhysicsBody(world, node, 0 /* STATIC */);
          setPhysicsBodyShape(world, body, shape);
