...
${varyBody}
  ...
  function applyPhysicsBodyImpulse(body, impulse, location2) {
    const hknp = body._world._hknp;
    hknp.HP_Body_ApplyImpulse(body._hkBody, [location2.x, location2.y, location2.z], [impulse.x, impulse.y, impulse.z]);
  }
  function applyPhysicsBodyForce(world, body, force, location2) {
    applyPhysicsBodyImpulse(body, { x: force.x * world._timestep, y: force.y * world._timestep, z: force.z * world._timestep }, location2);
  }
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene48.ts
  var PHYSICS_FPS = 60;
  var FRICTION = 0.2;
  var RESTITUTION = 0.3;
  var KICK_DELAY_MS = 2e3;
  var CAPTURE_STEPS_AFTER_KICK = 10;
  var KICK_FORCE = { x: 0, y: 0, z: 400 };
  var REST_Y = 2.05;
  var BOXES = [
  ...
  async function main() {
    ...
    const entries = BOXES.map((spec) => {
      ...
      const body = createPhysicsBody(world, mesh, 2 /* DYNAMIC */, true);
