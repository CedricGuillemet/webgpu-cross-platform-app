...
@fragment fn fs(v:V)->@location(0)vec4f{return textureSample(t,s,v.u);}`;
  ...
  function _registerSpriteFxHook(hook) {
    _spriteFxHook = hook;
  }
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/texture/pixels-texture.ts
  init_gpu_flags();
  init_gpu_pool();
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/sprite/custom-shader-core.ts
  init_typed_arrays();
  init_gpu_flags();
  init_gpu_buffers();
  var WGSL_NAME = /^[A-Za-z_][A-Za-z0-9_]*$/;
  var EMPTY_PARAMS = [0, 0, 0, 0];
...
@group(${group}) @binding(${binding}) var<uniform> fx: SpriteFx;`;
  ...
  var SPRITE_FX_UBO_BYTES = 32;
  var SPRITE_FX_UBO_FLOATS = SPRITE_FX_UBO_BYTES / 4;
  ...
  function makeShaderModuleCache() {
    ...
    return (engine, key, makeCode) => {
      devices ?? (devices = /* @__PURE__ */ new WeakMap());
...
return out;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/sprite/sprite-custom-shader.ts
  init_sprite_fx_hook();
...
${fragment}
  ...
  // ../../../Babylon-Lite/lab/lite/src/_shared/sprite-atlas-cutout.ts
  var ATLAS_WIDTH = 128;
  var ATLAS_HEIGHT = 64;
  var CELL = 32;
  var _cached = null;
  var CUTOUT_SPRITE_ATLAS_INFO = {
    widthPx: ATLAS_WIDTH,
  ...
  function getCutoutSpriteAtlasDataUrl() {
    if (_cached) {
      return _cached;
    }
  ...
  function drawCell(ctx, x, y, index) {
    switch (index) {
      case 0:
  ...
  // ../../../Babylon-Lite/lab/lite/src/_shared/sprite-grid.ts
  function addDeterministicSpriteGrid(layer, canvas, options) {
  ...
  // ../../../Babylon-Lite/lab/lite/src/_shared/palette-remap.ts
  var PALETTE_WIDTH = 256;
  var _cachedPalette = null;
  function buildColormapPalette() {
    if (_cachedPalette) {
    ...
    for (let i = 0; i < PALETTE_WIDTH; i++) {
      ...
      const r = 0.5 + 0.5 * Math.sin(twoPi * (t + 0));
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene93.ts
  async function main() {
