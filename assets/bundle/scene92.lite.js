...
@fragment fn fs(v:V)->@location(0)vec4f{return textureSample(t,s,v.u);}`;
  ...
  function setSprite2DShaderParams(layer, params) {
    const target = layer.shaderParams ?? (layer.shaderParams = [0, 0, 0, 0]);
    target[0] = params[0];
    target[1] = params[1];
    target[2] = params[2];
    target[3] = params[3];
...
${fragment}
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene92.ts
  var SCENE92_PARAMS = [1, 0.78, 0.55, 1];
  async function main() {
