...
${varyBody}
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/animation/weighted-pointer-mixer.ts
  init_typed_arrays();
  init_animation_group();
  ...
  function enablePropertyAnimationBlending(manager) {
  ...
  function fadeAnimationWeight(manager, group, options) {
    const to = validateWeight(options.to);
    const from = options.from === void 0 ? group.weight : validateWeight(options.from);
  ...
  function crossFadeAnimationGroups(manager, fromGroup, toGroup, options) {
  ...
  function updateWeightedPointerAnimations(manager, deltaMs) {
    const scratch = getScratch(manager);
  ...
  function updateFades(scratch, deltaMs) {
    for (let i = scratch.fades.length - 1; i >= 0; i--) {
  ...
  function advancePropertyGroupTime(group, mixer, deltaMs) {
    if (group.isPlaying) {
  ...
  function getTrackBucket(buckets, track) {
  ...
  function accumulateWeightedTrack(bucket, track, sample, weight) {
    bucket.active = true;
    let sign = 1;
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene156.ts
  var FRAME_RATE = 10;
  var END_FRAME = 2 * FRAME_RATE;
  var FADE_START_MS = 1e3;
