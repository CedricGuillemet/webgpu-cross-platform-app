...
(() => {
  ...
  var _nearestDesc, _bilinearDesc, _trilinearDesc;
  ...
  function createPostProcessGpuState(task, engine) {
    ...
    task._pipeline = engine._device.createRenderPipeline({
      ...
      fragment: {
        ...
        targets: [{ format, blend: alphaModeToBlend(task.alphaMode) }]
      },
      primitive: { topology: "triangle-list" },
...
${task._shader.fragmentWrapperWGSL ?? FRAGMENT_WRAPPER_WGSL}`;
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/post-process/blur.ts
  var MAX_VERTEX_BLUR_SAMPLES = 8;
  var BLUR_UNIFORM_WGSL = `struct BlurParams{delta:vec2f,p0:f32,p1:f32}
@group(0) @binding(2) var<uniform> blurParams:BlurParams;`;
  ...
  function updateBlurShader(shader, kernel) {
  ...
  function createBlurPostProcessTask(config, engine, scene) {
  ...
  // ../../../Babylon-Lite/packages/babylon-lite/src/post-process/chromatic-aberration.ts
  var CHROMATIC_ABERRATION_UNIFORM_WGSL = `struct ChromaticAberrationParams{chromatic_aberration:f32,screen_width:f32,screen_height:f32,radialIntensity:f32,direction:vec2f,centerPosition:vec2f}
@group(0) @binding(2) var<uniform> chromaticAberrationParams:ChromaticAberrationParams;`;
  var CHROMATIC_ABERRATION_FRAGMENT_WGSL = `fn applyPostProcess(color:vec4f, uv:vec2f)->vec4f{let centered=uv-chromaticAberrationParams.centerPosition;var dir=chromaticAberrationParams.direction;if(dir.x==0.0&&dir.y==0.0){dir=normalize(centered);}let radius=sqrt(dot(centered,centered));let amount=chromaticAberrationParams.chromatic_aberration*pow(radius,chromaticAberrationParams.radialIntensity);let shift=amount*dir/vec2f(chromaticAberrationParams.screen_width,chromaticAberrationParams.screen_height);let r=samplePostProcessSource(vec2f(uv.x+shift.x*-0.3,uv.y+shift.y*-0.3*0.5));let g=samplePostProcessSource(uv);let b=samplePostProcessSource(vec2f(uv.x+shift.x*0.3,uv.y+shift.y*0.3*0.5));return vec4f(r.r,g.g,b.b,clamp(r.a+g.a+b.a,0,1));}`;
  function createChromaticAberrationPostProcessTask(config, engine, scene) {
    const params = {
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene143.ts
  async function main() {
