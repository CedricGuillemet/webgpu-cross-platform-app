    ...
    "../../../Babylon-Lite/packages/babylon-lite/src/material/node/blocks/wave-block.ts"() {
      ...
      emitter61 = {
        ...
        emit(block, _outputName, stage, state, ctx) {
          ...
          if (kind === 2) {
            return { expr: `(${two} * abs(${two} * (${v} - floor(${half2} + ${v}))) - ${one3})`, type: input2.type };
          }
...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene79-nme.ts
  var TARGET_VERTEX = 1;
  ...
  function merge3(name, x, y, z, xOutput = "output", yOutput = "output", zOutput = "output") {
    return addBlock(
      "VectorMergerBlock",
      name,
      TARGET_NEUTRAL,
  ...
  function conditional(name, condition, a, b, trueValue, falseValue, aOutput = "output", bOutput = "output", trueOutput = "output", falseOutput = "output") {
    return addBlock(
      "ConditionalBlock",
      name,
      TARGET_NEUTRAL,
  ...
  var transform = addBlock("TransformBlock", "Transform", TARGET_VERTEX, [input("vector", { id: position, output: "output" }), input("transform", { id: wvp, output: "output" })], [output("output"), output("xyz")], { complementZ: 0, complementW: 1 });
  ...
  var tenth = inputBlock("tenth", TYPE_FLOAT, 0.1);
  var quarter = inputBlock("quarter", TYPE_FLOAT, 0.25);
  var third = inputBlock("third", TYPE_FLOAT, 0.3333333333333333);
  var half = inputBlock("half", TYPE_FLOAT, 0.5);
  var twoThirds = inputBlock("twoThirds", TYPE_FLOAT, 0.6666666666666666);
  var nineTenths = inputBlock("nineTenths", TYPE_FLOAT, 0.9);
  var one2 = inputBlock("one", TYPE_FLOAT, 1);
  var three = inputBlock("three", TYPE_FLOAT, 3);
  var four = inputBlock("four", TYPE_FLOAT, 4);
  var nlerpLeft = inputBlock("nlerpLeft", TYPE_VECTOR3, [1, 0.15, 0.05]);
  var nlerpRight = inputBlock("nlerpRight", TYPE_VECTOR3, [0.05, 0.2, 1]);
  var nlerp = addBlock("NLerpBlock", "NLerpGradient", TARGET_NEUTRAL, [input("left", { id: nlerpLeft, output: "output" }), input("right", { id: nlerpRight, output: "output" }), input("gradient", { id: uvSplit, output: "x" })], [output("output")]);
  var nlerpSplit = split("SplitNLerp", nlerp, "output", "xyz");
  var condLess = conditional("CondLessThanStripe", 2, uvSplit, third, tenth, nineTenths, "x");
  var condGreaterX = conditional("CondGreaterX", 3, uvSplit, half, one2, zero, "x");
  var condGreaterY = conditional("CondGreaterY", 3, uvSplit, half, one2, zero, "y");
  var condAnd = conditional("CondAndQuadrants", 8, condGreaterX, condGreaterY, one2, quarter);
  var rSumA = binary("AddBlock", "RAddNLerpLess", nlerpSplit, condLess, "x");
  var rSumB = binary("AddBlock", "RAddAnd", rSumA, condAnd);
  var red = binary("DivideBlock", "RedAverage", rSumB, three);
  var curveInOutSine = curve("CurveEaseInOutSineX", 2, uvSplit, "x");
  var curveInQuad = curve("CurveEaseInQuadY", 3, uvSplit, "y");
  var curveInBack = curve("CurveEaseInBackX", 21, uvSplit, "x");
  var condLe = conditional("CondCurveLessOrEqual", 4, uvSplit, twoThirds, curveInQuad, curveInBack, "x");
  var gSumA = binary("AddBlock", "GAddSineConditional", curveInOutSine, condLe);
  var gSumB = binary("AddBlock", "GAddQuad", gSumA, curveInQuad);
  var green = binary("DivideBlock", "GreenAverage", gSumB, three);
  var xTimesFour = binary("MultiplyBlock", "XTimesFour", uvSplit, four, "x");
  var yTimesFour = binary("MultiplyBlock", "YTimesFour", uvSplit, four, "y");
  var xPlusY = binary("AddBlock", "XPlusY", uvSplit, uvSplit, "x", "y");
  var diagonalTimesThree = binary("MultiplyBlock", "DiagonalTimesThree", xPlusY, three);
  var saw = wave("WaveSawX", 0, xTimesFour);
  var square = wave("WaveSquareY", 1, yTimesFour);
  var triangle = wave("WaveTriangleDiag", 2, diagonalTimesThree);
  var sawMapped = binary("AddBlock", "SawMap", binary("MultiplyBlock", "SawHalf", saw, half), half);
  var squareMapped = binary("AddBlock", "SquareMap", binary("MultiplyBlock", "SquareHalf", square, half), half);
  var triangleMapped = binary("AddBlock", "TriangleMap", binary("MultiplyBlock", "TriangleHalf", triangle, half), half);
  var random = addBlock("RandomNumberBlock", "RandomFromUV", TARGET_NEUTRAL, [input("seed", { id: uv, output: "output" })], [output("output")]);
  var bSumA = binary("AddBlock", "BAddSawSquare", sawMapped, squareMapped);
  var bSumB = binary("AddBlock", "BAddTriangle", bSumA, triangleMapped);
  var bSumC = binary("AddBlock", "BAddRandom", bSumB, random);
  var blue = binary("DivideBlock", "BlueAverage", bSumC, four);
  var color = merge3("MergeModesColor", red, green, blue);
  var fragmentOutput = addBlock("FragmentOutputBlock", "FragmentOutput", TARGET_FRAGMENT, [input("rgba"), input("rgb", { id: color, output: "xyz" }), input("a")], [], {
    convertToGammaSpace: false,
    convertToLinearSpace: false,
    useLogarithmicDepth: false
  });
  var SCENE79_NME_JSON = {
    tags: null,
    ignoreAlpha: false,
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene79.ts
  async function main() {
