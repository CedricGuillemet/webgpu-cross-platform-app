...
`;
  ...
  // ../../../Babylon-Lite/lab/lite/src/shared/scene66-nme.ts
  var SCENE66_MORPH_PERIOD_MS = 6283.185;
  ...
  function sphereScrambleDeltas(vertexCount, seed = 2882400001) {
  ...
  function getScene66TextureInfos(json) {
    const textures = [];
    const blocks = json.blocks ?? [];
    for (const b of blocks) {
      const ct = b["customType"] ?? "";
  ...
  function createScene66FinalAlphaDiscardJson(json, cutoff = 0.4) {
    ...
    for (const input of discard.inputs) {
      if (input.name === "value") {
  ...
  // ../../../Babylon-Lite/lab/lite/src/lite/scene140.ts
  async function main() {
    ...
    light.shadowGenerator = sg ?? void 0;
