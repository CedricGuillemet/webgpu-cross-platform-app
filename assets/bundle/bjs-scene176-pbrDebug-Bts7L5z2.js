import{S as e}from"./bjs-scene176.js";import"./bjs-scene176-mainUVVaryingDeclaration-Bs1iEPxZ.js";import"./bjs-scene176-logDepthFragment-PPN6ETpj.js";const t="pbrFragmentExtraDeclaration",g=`varying vec3 vPositionW;
#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=g);const o="subSurfaceScatteringFunctions",u=`bool testLightingForSSS(float diffusionProfile)
{return diffusionProfile<1.;}`;e.IncludesShadersStore[o]||(e.IncludesShadersStore[o]=u);const i="importanceSampling",E=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=E);const a="pbrHelperFunctions",h=`#define MINIMUMVARIANCE 0.0005
...
#endif
`;e.IncludesShadersStore[a]||(e.IncludesShadersStore[a]=h);const r="pbrDirectLightingSetupFunctions",L=`struct preLightingInfo
...
result.areaLightDiffuse=data.Diffuse;result.LdotV=0.;result.roughness=0.;result.diffuseRoughness=0.;result.surfaceAlbedo=vec3(0.);return result;}
#endif
`;e.IncludesShadersStore[r]||(e.IncludesShadersStore[r]=L);const n="pbrDirectLightingFalloffFunctions",p=`float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
#endif
}`;e.IncludesShadersStore[n]||(e.IncludesShadersStore[n]=p);const l="hdrFilteringFunctions",D=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;e.IncludesShadersStore[l]||(e.IncludesShadersStore[l]=D);const f="pbrDirectLightingFunctions",v=`#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
...
#endif
`;e.IncludesShadersStore[f]||(e.IncludesShadersStore[f]=v);const s="pbrBlockNormalGeometric",m=`vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
#endif
`;e.IncludesShadersStore[s]||(e.IncludesShadersStore[s]=m);const c="pbrBlockImageProcessing",A=`#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
#endif
`;e.IncludesShadersStore[c]||(e.IncludesShadersStore[c]=A);const d="pbrDebug",C=`#if DEBUGMODE>0
