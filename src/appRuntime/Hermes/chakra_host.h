...
public:
    ...
    JsRuntimeHandle runtime() const { return runtimePtr_; }
    JsContextRef    context() const { return nullptr; }
    const std::string& rootBaseDir() const { return rootBaseDir_; }

private:
    std::unique_ptr<facebook::hermes::HermesRuntime> runtimeImpl_;
    facebook::jsi::Runtime* runtimePtr_ = nullptr;
